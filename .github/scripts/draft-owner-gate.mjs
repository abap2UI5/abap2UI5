...
// (UNAME on Z2UI5_T_01, fail-closed). Three read paths deliberately tolerate
// a BLANK owner - rows written before the column existed - so no active
...
// the three tolerated blank-owner reads: read( ), check_exists( ),
// count_entries( ) - one anchor each, spelled exactly as the source does
const MARKERS = [
...
if (!pastGrace) {
  if (present.length !== MARKERS.length) {
    ...
    console.log("");
    console.log("The tolerated blank-owner reads moved or were rewritten. If they were");
  ...
  if (present.length > 0) {
    ...
    console.log("Remove the three tolerances (read/check_exists/count_entries + the comment");
    console.log("block naming this removal condition), then delete this gate.");
    process.exit(1);
  }
