...
13. **The app runs under a CSP without `'unsafe-eval'`-free assumptions — keep it eval-capable and avoid eval-only UI5 features.** The default CSP (in `z2ui5_cl_ui5_user_exit`) keeps `'unsafe-eval'` because the OpenUI5 **1.71** ui5loader evals module source; removing it breaks the 1.71 bootstrap with a CSP `EvalError`. UI5 **expression binding** (`{= … }`) is not one of the eval users: UI5 parses it with its own `sap/ui/base/ExpressionParser` (a Pratt parser — no `eval`/`new Function` in 1.71 through 1.120, checked in the npm sources and in a browser under a CSP without `'unsafe-eval'`), so it is fine in framework XML too. What does need `'unsafe-eval'` is a module arriving as a source string — synchronous loading, or a resource missing from the preload bundles.
