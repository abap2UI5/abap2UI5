...
│   ├── z2ui5_if_exit.intf.abap         # Superseded name of the above - still shipped, still called
...
5. **The public API (`src/02/`) is a stable contract — never change or remove existing public attributes, methods, or constants.** This folder is consumed directly by thousands of downstream apps. Specifically:
   - Do not rename, remove, or change the signature of any method in `z2ui5_if_client`, `z2ui5_if_app`, `z2ui5_if_ui5_exit`, or `z2ui5_if_exit`
   ...
   - The recorded exceptions, all owner-approved; 1-4 come from the move to the `ui5` namespace, 5 from retiring the shared type interface. Not a precedent for editing the snapshot on any other finding — 1 and 2 are `CHANGED`, 3 is the far heavier `REMOVED`:
     ...
     6. `z2ui5_if_exit` renamed to `z2ui5_if_ui5_exit` **without** an incompatibility: 3 `CHANGED` and 5 additions, 0 removals. The old interface stays, ships and is still looked up, so every existing exit keeps working; what changed is that its three types are now declared as references to the ones on the new interface (`types ty_s_http_config type z2ui5_if_ui5_exit=>ty_s_http_config`) instead of repeating the definition. Same technical type, so a variable declared either way fits either signature - and one definition rather than two that can drift apart while the framework hands the same structure to both
