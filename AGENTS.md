...
└── 99/   FROZEN legacy code. Legacy XML view builder (z2ui5_cl_xml_view / _cc) + the retired z2ui5_cl_ai_xml, the deprecated z2ui5_cl_http_handler shim, z2ui5_if_types, retired z2ui5_cl_util* classes (99/01) and popups (99/02). Ships so existing downstream installations keep compiling. Its test classes are the exception: they run in CI and guard the layer
```
...
- **Layer 2 (`src/02/`)** — Public API. The stable contract for app developers. Six objects: `z2ui5_if_app`, `z2ui5_if_client`, `z2ui5_if_exit`, `z2ui5_cl_ui5_http_handler` (the HTTP entry point), and the view builder `z2ui5_cl_ui5_view_builder`. Recorded symbol for symbol in `.github/api-snapshot.json` (rule 5).
- **Package `src/99/` — frozen legacy code.** It ships so **existing downstream installations** keep compiling on upgrade. Its production code has **one** in-repo consumer, and it is a known wart rather than a pattern to copy: `z2ui5_if_types` moved here with the `ui5` rename, yet `src/02/z2ui5_if_exit` and `src/02/z2ui5_if_client` still name its types in public signatures, and `src/01` reads them too — which is why `abaplint.jsonc` has to *parse* `src/99` (under `noIssues`, so it is parsed but never judged) for those types to resolve. Nothing else in the repo references the package. Its **test classes are live**, though: they lint and run in the transpiled unit suite (`npm run unit`), guarding the layer against regressions — which is why they, unlike the production code, may change (they assert against core internals such as `t_action_front` and follow them when those move):
  - **Package top level** — the legacy XML view builder (`z2ui5_cl_xml_view`, `z2ui5_cl_xml_view_cc`) and, since the `ui5` rename, `z2ui5_cl_ai_xml` (which superseded them and was in turn superseded by `z2ui5_cl_ui5_view_builder` in `src/02/`), the deprecated `z2ui5_cl_http_handler` shim that forwards to `z2ui5_cl_ui5_http_handler`, and `z2ui5_if_types`. All builder work happens in `z2ui5_cl_ui5_view_builder`.
...
└── 99/                        # HISTORY ONLY - ignore, the only consumer is z2ui5_if_types (see "Layered Design")
    ...
    ├── z2ui5_if_types.intf.abap        # Shared type definitions - still named by src/02 + src/01 signatures
...
| File | Why |
|---|---|
| `src/99/z2ui5_if_types.intf.abap` | Shared type definitions — still named by `src/02` and `src/01` signatures despite sitting in the frozen package |
...
5. **The public API (`src/02/`) is a stable contract — never change or remove existing public attributes, methods, or constants.** This folder is consumed directly by thousands of downstream apps. Specifically:
   - Do not rename, remove, or change the signature of any method in `z2ui5_if_client`, `z2ui5_if_app`, or `z2ui5_if_exit` — nor in `z2ui5_if_types`, which sits in `src/99` but is named by public `src/02` signatures and is therefore contract in everything but its folder (the snapshot only scans `src/02`, so nothing guards it — treat it by hand)
