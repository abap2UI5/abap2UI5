...
| `.github/shared/` | Files this repository is the SOURCE of for other repositories but does not use itself — today `agents-metadata.md`, the metadata convention `samples`, `samples-controls` and `samples-stack` each carry inside their `AGENTS.md`. A shared file needs one owner and this is the repository the others already depend on; `shared-file-gate.mjs` is what notices when a copy stops matching |
