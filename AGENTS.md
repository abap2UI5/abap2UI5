...
| **Frontend delivery** | `frontend_check.yaml`, `frontend_deploy.yaml` | `frontend_check.yaml` is the pre-merge gate the two-repository split made impossible: on every pull request touching `app/webapp/`, `frontend/` or `tools/` it builds all branch variants from the webapp being changed, lints the generated ABAP and checks the BSP page invariants (`tools/check-pages.mjs`). `frontend_deploy.yaml` builds one branch and pushes it into the delivery repository via deploy key (`ACTION_KEY_FRONTEND`) — dispatch-only until the delivery repository stops building the branches itself |
