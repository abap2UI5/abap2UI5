...
CLASS z2ui5_cl_ui5_http_handler IMPLEMENTATION.
  ...
  METHOD main.
            DATA temp21 TYPE z2ui5_if_ui5_exit=>ty_s_http_config_post.
            DATA ls_config_post LIKE temp21.
  ...
  METHOD check_csrf_rejected_request.
    ...
    IF is_config-check_trust_forwarded_host = abap_true.
      temp22 = mo_server->get_header_field( `x-forwarded-host` ).
    ELSE.
      CLEAR temp22.
    ENDIF.
  ...
  METHOD _http_get.
    ...
    IF ls_config-styles_css IS INITIAL.
      temp25 = z2ui5_cl_ui5f_style_css=>get( ).
    ELSE.
      temp25 = ls_config-styles_css.
    ENDIF.
...
DATA lt_candidate TYPE temp1.
  ...
  METHOD set_response.
    ...
    DATA lv_content_type LIKE temp27.
    ...
    IF ms_res-status_code >= 400.
      temp27 = `text/plain; charset=UTF-8`.
    ELSEIF ms_req-method = `GET`.
      temp27 = `text/html; charset=UTF-8`.
    ELSE.
      temp27 = `application/json; charset=UTF-8`.
    ENDIF.

    lv_content_type = temp27.
  ...
  METHOD _error_response.
    ...
    DATA temp28 TYPE z2ui5_if_ui5_exit=>ty_s_http_config_post.
    DATA ls_config_post LIKE temp28.
    DATA lv_body TYPE string.
    CLEAR temp28.

    ls_config_post = temp28.
    TRY.
        z2ui5_cl_ui5_user_exit=>get_instance( )->set_config_http_post( CHANGING cs_config = ls_config_post ).
      CATCH cx_root ##NO_HANDLER.
