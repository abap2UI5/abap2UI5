...
CLASS z2ui5_cl_ui5_http_handler IMPLEMENTATION.
  ...
  METHOD main.
            DATA temp19 TYPE z2ui5_if_ui5_exit=>ty_s_http_config_post.
            DATA ls_config_post LIKE temp19.
  ...
  METHOD check_csrf_rejected_request.
    ...
    IF is_config-check_trust_forwarded_host = abap_true.
      temp20 = mo_server->get_header_field( `x-forwarded-host` ).
    ELSE.
      CLEAR temp20.
    ENDIF.
  ...
  METHOD _http_get.
    ...
    DATA temp22 LIKE LINE OF ls_config-t_add_config.
    DATA lr_config_key LIKE REF TO temp22.
      DATA lv_etag LIKE sv_get_etag.
    DATA lv_if_none_match LIKE sv_if_none_match.
    DATA temp23 TYPE string.
    DATA lv_style_css LIKE temp23.
    DATA lv_preload TYPE string.
    DATA lv_globals TYPE string.
    DATA lv_add_config TYPE string.
    DATA temp24 LIKE LINE OF ls_config-t_add_config.
    DATA lr_config LIKE REF TO temp24.
    ...
    IF ls_config-styles_css IS INITIAL.
      temp23 = z2ui5_cl_ui5f_style_css=>get( ).
    ELSE.
      temp23 = ls_config-styles_css.
    ENDIF.
...
DATA lt_candidate TYPE temp1.
  ...
  METHOD set_response.
    ...
    DATA lv_content_type LIKE temp25.
    ...
    IF ms_res-status_code >= 400.
      temp25 = `text/plain; charset=UTF-8`.
    ELSEIF ms_req-method = `GET`.
      temp25 = `text/html; charset=UTF-8`.
    ELSE.
      temp25 = `application/json; charset=UTF-8`.
    ENDIF.

    lv_content_type = temp25.
  ...
  METHOD _error_response.
    ...
    DATA temp26 TYPE z2ui5_if_ui5_exit=>ty_s_http_config_post.
    DATA ls_config_post LIKE temp26.
    DATA lv_body TYPE string.
    CLEAR temp26.

    ls_config_post = temp26.
    TRY.
        z2ui5_cl_ui5_user_exit=>get_instance( )->set_config_http_post( CHANGING cs_config = ls_config_post ).
      CATCH cx_root ##NO_HANDLER.
