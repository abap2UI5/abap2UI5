...
CLASS z2ui5_cl_ui5_srv_event IMPLEMENTATION.
  ...
  METHOD get_event.
    ...
    temp63 = val.
    result = |{ lv_func }({ lv_event_arg }['{ escape_js_string( temp63 ) }'|.
    ...
    IF s_cnt-check_queue_last = abap_true OR s_cnt-check_no_busy = abap_true.
      ...
      ELSE.
        temp64 = `false`.
      ENDIF.
      result = |{ result },false,false,false,{ temp64 }|.
  ...
  METHOD map_client_event.
    ...
    DATA lv_slot LIKE temp66.
      ...
      DATA lv_bind_path LIKE temp72.
      DATA temp74 TYPE string_table.
      DATA temp10 LIKE LINE OF temp74.
    ...
    IF lv_slot IS NOT INITIAL.
      ...
      IF sy-subrc = 0.
        temp4 = temp5.
      ENDIF.
      INSERT temp4 INTO TABLE temp67.
      INSERT lv_slot INTO TABLE temp67.
      INSERT `to` INTO TABLE temp67.
      ...
      IF sy-subrc = 0.
        temp6 = temp7.
      ENDIF.
      INSERT temp6 INTO TABLE temp67.
      lt_arg = temp67.
    ...
    ELSEIF lv_val = z2ui5_if_client=>cs_event-popup_close
      ...
      CLEAR temp69.
      INSERT z2ui5_if_ui5_types=>cs_slot_action-target INTO TABLE temp69.
      INSERT z2ui5_if_ui5_types=>cs_slot_action-destroy INTO TABLE temp69.
      ...
      ENDIF.
      INSERT temp8 INTO TABLE temp69.
      lt_arg = temp69.
    ...
    ELSEIF lv_val = z2ui5_if_client=>cs_event-bind_element.
      ...
      READ TABLE t_arg INTO temp73 INDEX 2.
      IF sy-subrc = 0.
        temp72 = temp73.
      ENDIF.
      ...
      temp10 = view.
      INSERT temp10 INTO TABLE temp74.
      ...
      IF sy-subrc = 0.
        temp11 = temp12.
      ENDIF.
      INSERT temp11 INTO TABLE temp74.
      INSERT lv_bind_path INTO TABLE temp74.
      lt_arg = temp74.
    ENDIF.
  ...
  METHOD get_event_client_ajson.
    ...
    DATA lv_index TYPE i.
      FIELD-SYMBOLS <temp76> LIKE LINE OF ls_event-t_arg.
      DATA temp77 LIKE sy-tabix.
        DATA temp78 TYPE REF TO z2ui5_if_ajson.
        DATA li_json LIKE temp78.
    ...
    WHILE lv_index > 0.
      ...
      temp77 = sy-tabix.
      READ TABLE ls_event-t_arg INDEX lv_index ASSIGNING <temp76>.
      sy-tabix = temp77.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      IF <temp76> IS NOT INITIAL.
        EXIT.
      ENDIF.
