...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD event_view_param.
    ...
    CLEAR temp13.
    INSERT `demoPanel` INTO TABLE temp13.
    INSERT `setExpanded` INTO TABLE temp13.
    INSERT `X` INTO TABLE temp13.
    cl_abap_unit_assert=>assert_equals(
    ...
    CLEAR temp15.
    INSERT `demoPanel` INTO TABLE temp15.
    INSERT `setExpanded` INTO TABLE temp15.
    INSERT `X` INTO TABLE temp15.
    cl_abap_unit_assert=>assert_equals(
        exp = `.eF('CONTROL_BY_ID', 'demoPanel', '', 'setExpanded', 'X')`
        act = lo_event->get_event_client( val   = z2ui5_if_client=>cs_event-control_by_id
  ...
  METHOD event_prevent_default_expr.
    ...
    CLEAR temp19.
    INSERT `${$parameters>/width}` INTO TABLE temp19.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD event_quote_escaped.
    ...
    CLEAR temp21.
    INSERT `Value changed to '{0}'` INTO TABLE temp21.
  ...
  METHOD event_backslash_escaped.
    ...
    CLEAR temp23.
    INSERT `\',alert(1),'` INTO TABLE temp23.
    ...
    temp19 = boolc( lv_event CS `'\\\',alert(1),\''` ).
  ...
  METHOD event_lone_cr_escaped.
    ...
    DATA temp3 LIKE LINE OF temp25.
    DATA lt_arg LIKE temp25.
    DATA lv_event TYPE string.
    ...
    INSERT temp3 INTO TABLE temp25.
    ...
    temp21 = boolc( lv_event CS `'before\rafter'` ).
    cl_abap_unit_assert=>assert_true( temp21 ).

    temp22 = boolc( lv_event CS lv_cr ).
  ...
  METHOD event_placeholder_quoted.
    ...
    CLEAR temp27.
    INSERT `{0} Pressed` INTO TABLE temp27.

    lv_plain = lo_event->get_event( val   = `EVT`
                                          t_arg = temp27 ).

    temp23 = boolc( lv_plain CS `'{0} Pressed'` ).
    cl_abap_unit_assert=>assert_true( temp23 ).


    CLEAR temp29.
    INSERT `{0?Pressed:Unpressed}` INTO TABLE temp29.

    lv_cond = lo_event->get_event( val   = `EVT`
                                         t_arg = temp29 ).

    temp24 = boolc( lv_cond CS `'{0?Pressed:Unpressed}'` ).
  ...
  METHOD json_basic.
    ...
    CLEAR temp31.
    INSERT `My Title` INTO TABLE temp31.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD json_nav_container.
    ...
    CLEAR temp33.
    INSERT `myContainer` INTO TABLE temp33.
    INSERT `myPage` INTO TABLE temp33.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD json_object_arg.
    ...
    CLEAR temp41.
    INSERT `{"KEY":"K1"}` INTO TABLE temp41.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD json_escaping.
    ...
    CLEAR temp45.
    INSERT `he said "hi" \ bye` INTO TABLE temp45.
    cl_abap_unit_assert=>assert_equals(
