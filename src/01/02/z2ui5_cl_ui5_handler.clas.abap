CLASS z2ui5_cl_ui5_handler DEFINITION PUBLIC FINAL.
  ...
  PRIVATE SECTION.
    ...
    " otherwise loop the work process forever. Writable instance DATA, not a
    " CONSTANTS like the event-argument limit below, because the loop-guard
    " test lowers it (ltcl_03_dispatch): the alternative is 1000 real hops,
    " each of them a draft save. Nothing in production writes it
...
CLASS z2ui5_cl_ui5_handler IMPLEMENTATION.
  ...
  METHOD request_json_to_abap.
    ...
    TRY.
        ...
        ELSE.
          ...
          temp1 = boolc( result-s_control-app_start_draft IS NOT INITIAL ).
          result-s_control-check_app_state = temp1.
        ENDIF.
  ...
  METHOD request_parse_body.
    ...
    DATA lo_ajson LIKE temp42.
    DATA temp43 TYPE string.
    DATA lv_root LIKE temp43.
    DATA lv_model_path TYPE string.
    DATA lv_front TYPE string.
    ...
    temp42 ?= z2ui5_cl_ajson=>parse( val ).
  ...
  METHOD request_parse_config.
    DATA lv_device TYPE string.
    DATA lv_focus TYPE string.
    DATA lv_scroll TYPE string.
    DATA lv_ui5 TYPE string.
  ...
  METHOD request_parse_event_args.
    ...
    DATA lv_arg_index TYPE i.
      ...
      DATA lv_node_type TYPE z2ui5_if_ajson_types=>ty_node_type.
          DATA temp44 LIKE LINE OF result.
    lv_arg_index = 1.
    DO.

      lv_arg_path = |{ iv_front }/T_EVENT_ARG/{ lv_arg_index }|.

      lv_node_type = io_json->get_node_type( lv_arg_path ).
      IF lv_node_type = ``.
        EXIT.
      ENDIF.
      ...
      CASE lv_node_type.
        ...
        WHEN z2ui5_if_ajson_types=>node_type-boolean.
          ...
          temp44 = io_json->get_boolean( lv_arg_path ).
          APPEND temp44 TO result.
        WHEN OTHERS.
  ...
  METHOD hash_get_shell_part.
    ...
    hash_split( EXPORTING iv_hash      = iv_hash
                IMPORTING ev_hash      = lv_hash
                          ev_off       = lv_off
                          ev_check_app = lv_check_app ).
  ...
  METHOD app_get_url.
    ...
    DATA temp45 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
    DATA temp46 TYPE string.
    DATA lv_shell TYPE string.
    DATA temp47 TYPE string.
    DATA lv_hash LIKE temp47.
    lt_param = z2ui5_cl_ui5_util_context=>url_param_get_tab( search ).
    DELETE lt_param WHERE n = `app_start`.

    CLEAR temp45.
    temp45-n = `app_start`.
    temp45-v = to_lower( classname ).
    INSERT temp45 INTO TABLE lt_param.
    ...
    lv_shell = hash_get_shell_part( iv_hash             = temp46
  ...
  METHOD route_split.
    ...
    IF strlen( lv_rest ) > 1 AND lv_rest(1) = `/`.
      ...
      lv_tail = substring( val = lv_rest
                                 off = 1 ).

      lv_ns_end = find( val = lv_tail
                              sub = `/` ).
  ...
  METHOD response_abap_to_json.
        ...
        DATA li_ajson_result LIKE temp48.
    ...
    TRY.
        ...
        " the shared upper-case mapping, not one built here: it is stateless,
        " and this method runs once per response - see
        " z2ui5_cl_ui5_util_json_fl, which holds the one instance and the
        " no-empty-values filter below with it

        temp48 ?= z2ui5_cl_ajson=>create_empty( ii_custom_mapping = z2ui5_cl_ui5_util_json_fl=>mapper_upper( ) ).

        li_ajson_result = temp48.
        ...
        lv_frontend = li_ajson_result->stringify( ).
        ...
        IF val-model IS INITIAL OR val-model = `{}`.
          temp49 = |\{"S_FRONT":{ lv_frontend }\}|.
        ELSE.
          temp49 = |\{"S_FRONT":{ lv_frontend },"MODEL":{ val-model }\}|.
        ENDIF.
        result = temp49.
  ...
  METHOD request_context_info.
    ...
    lv_url = context_info_cap( ms_request-s_front-pathname && ms_request-s_front-search ).
    ...
    lv_app_start = z2ui5_cl_ui5_action=>app_start_safe( ms_request-s_control-app_start ).


    IF lv_app IS NOT INITIAL.
      temp50 = | in app { lv_app }|.
    ELSE.
      CLEAR temp50.
    ENDIF.
  ...
  METHOD launchpad_derive.
    ...
    temp2 = boolc( ms_request-s_front-search CS `scenario=LAUNCHPAD` OR ms_request-s_front-pathname CS `/ui2/flp` OR ms_request-s_front-pathname CS `test/flpSandbox` ).
    ms_request-s_control-check_launchpad = temp2.
  ...
  METHOD actions_serialize.
    DATA temp51 LIKE LINE OF t_action.
    DATA lr_action LIKE REF TO temp51.
  ...
  METHOD main_end.
    ...
    temp3 = boolc( temp5 = 0 ).
    lv_check_display = temp3. "#EC CI_SORTSEQ
  ...
  METHOD main_process.
    ...
    DATA temp52 TYPE REF TO z2ui5_if_client.
    DATA li_client LIKE temp52.
    DATA temp53 TYPE REF TO z2ui5_if_app.
    DATA li_app LIKE temp53.
    CREATE OBJECT temp52 TYPE z2ui5_cl_ui5_client EXPORTING ACTION = mo_action.

    li_client = temp52.

    temp53 ?= mo_action->mo_app->mo_app.
