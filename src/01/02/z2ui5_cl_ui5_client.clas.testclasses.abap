...
DATA lt_row TYPE temp3.
  ...
  METHOD test_omit_paths_wired.
    ...
    DATA li_client TYPE REF TO z2ui5_if_client.
    DATA temp91 TYPE string_table.
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    FIELD-SYMBOLS <temp93> TYPE z2ui5_if_ui5_types=>ty_s_attri.
    DATA li_stored LIKE lr_attri->custom_filter.
    DATA temp94 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp95 TYPE z2ui5_if_ajson_types=>ty_node.
    li_client ?= mo_client.


    CLEAR temp91.
    INSERT `MIN` INTO TABLE temp91.
    li_client->_bind( val                = mo_test_app->mv_name
                      omit_initial_paths = temp91 ).

    " the reference first, then the component: the v702 downport does not
    " lower `REF #( tab[ ] )->comp` as one expression


    READ TABLE mo_action->mo_app->mt_attri->* WITH KEY name = `MV_NAME` ASSIGNING <temp93>.
IF sy-subrc <> 0.
  ASSERT 1 = 0.
ENDIF.
GET REFERENCE OF <temp93> INTO lr_attri.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_false
        act = li_stored->keep_node( temp94 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_true
        act = li_stored->keep_node( temp95 ) ).
  ...
  METHOD test_omit_paths_and_filter.
    ...
    DATA li_client TYPE REF TO z2ui5_if_client.
    DATA temp96 TYPE REF TO z2ui5_if_ajson_filter.
    DATA li_custom LIKE temp96.
    DATA temp97 TYPE string_table.
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    FIELD-SYMBOLS <temp99> TYPE z2ui5_if_ui5_types=>ty_s_attri.
    DATA li_stored LIKE lr_attri->custom_filter.
    DATA temp100 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp101 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp102 TYPE z2ui5_if_ajson_types=>ty_node.
    li_client ?= mo_client.


    CREATE OBJECT temp96 TYPE ltcl_good_filter.

    li_custom = temp96.


    CLEAR temp97.
    INSERT `MIN` INTO TABLE temp97.
    li_client->_bind( val                = mo_test_app->mv_name
                      custom_filter      = li_custom
                      omit_initial_paths = temp97 ).

    " the reference first, then the component: the v702 downport does not
    " lower `REF #( tab[ ] )->comp` as one expression


    READ TABLE mo_action->mo_app->mt_attri->* WITH KEY name = `MV_NAME` ASSIGNING <temp99>.
IF sy-subrc <> 0.
  ASSERT 1 = 0.
ENDIF.
GET REFERENCE OF <temp99> INTO lr_attri.
    ...
    temp100-name = `DROPME`.
    temp100-type = `str`.
    temp100-value = `filled`.
    cl_abap_unit_assert=>assert_equals(
        exp = abap_false
        act = li_stored->keep_node( temp100 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_false
        act = li_stored->keep_node( temp101 ) ).
  ...
  METHOD test_omit_initial_db_save.
    ...
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    FIELD-SYMBOLS <temp103> TYPE z2ui5_if_ui5_types=>ty_s_attri.
    ...
    READ TABLE lo_cont_db->mt_attri->* WITH KEY name = `MV_NAME` ASSIGNING <temp103>.
IF sy-subrc <> 0.
  ASSERT 1 = 0.
ENDIF.
GET REFERENCE OF <temp103> INTO lr_attri.
  ...
  METHOD test_bind_tab_cell.
    ...
    DATA li_client TYPE REF TO z2ui5_if_client.
    DATA temp104 TYPE ltcl_test_app=>ty_s_emp.
    DATA temp105 TYPE ltcl_test_app=>ty_s_emp.
    FIELD-SYMBOLS <temp106> LIKE LINE OF mo_test_app->mt_emp.
    ...
    CLEAR temp104.
    temp104-name = `Michael Adams`.
    temp104-job = `Scrum Master`.
    INSERT temp104 INTO TABLE mo_test_app->mt_emp.

    CLEAR temp105.
    temp105-name = `John Miller`.
    temp105-job = `Product Owner`.
    INSERT temp105 INTO TABLE mo_test_app->mt_emp.



    temp107 = sy-tabix.
    READ TABLE mo_test_app->mt_emp INDEX 1 ASSIGNING <temp106>.
    sy-tabix = temp107.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
        exp = `{/MT_EMP/0/NAME}`
        act = li_client->_bind( val       = <temp106>-name
    ...
    temp109 = sy-tabix.
    READ TABLE mo_test_app->mt_emp INDEX 2 ASSIGNING <temp108>.
    sy-tabix = temp109.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
        exp = `{/MT_EMP/1/JOB}`
        act = li_client->_bind( val       = <temp108>-job
                                tab       = mo_test_app->mt_emp
                                tab_index = 2 ) ).
  ...
  METHOD test_bind_tab_cell_assign.
    ...
    DATA temp110 TYPE ltcl_test_app=>ty_s_emp.
    DATA temp111 TYPE ltcl_test_app=>ty_s_emp.

    li_client ?= mo_client.

    CLEAR temp110.
    temp110-name = `Michael Adams`.
    temp110-job = `Scrum Master`.
    INSERT temp110 INTO TABLE mo_test_app->mt_emp.

    CLEAR temp111.
    temp111-name = `John Miller`.
    temp111-job = `Product Owner`.
    INSERT temp111 INTO TABLE mo_test_app->mt_emp.
...
CLASS ltcl_test_model_skipped IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    DATA lo_http TYPE REF TO z2ui5_cl_ui5_handler.
    DATA temp112 TYPE ltcl_app_price_editor=>ty_s_pos.
    ...
    CLEAR temp112.
    temp112-qty = 1.
    APPEND temp112 TO ls_product-t_pos.
  ...
  METHOD test_accepted_price_silent.
    ...
    ENDIF.
    temp5 = <temp1>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp113
                                        act = temp5 ).
  ...
  METHOD test_refused_scalar_reported.
    ...
    FIELD-SYMBOLS <temp114> LIKE LINE OF lt_skipped.
    ...
    temp115 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp114>.
    sy-tabix = temp115.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    cl_abap_unit_assert=>assert_equals( exp = temp120
                                        act = temp6 ).

  ENDMETHOD.

  METHOD test_refused_price_reported.
    DATA temp121 TYPE decfloat34.
    ...
    ENDIF.
    temp7 = <temp3>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp121
                                        act = temp7 ).
  ...
  METHOD test_nested_row_unresolved.
    FIELD-SYMBOLS <temp124> LIKE LINE OF mo_app->mt_product.
    DATA temp125 LIKE sy-tabix.
    FIELD-SYMBOLS <temp10> LIKE LINE OF <temp124>-t_pos.
    ...
    temp125 = sy-tabix.
    READ TABLE mo_app->mt_product INDEX 1 ASSIGNING <temp124>.
    sy-tabix = temp125.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp11 = sy-tabix.
    READ TABLE <temp124>-t_pos INDEX 1 ASSIGNING <temp10>.
    sy-tabix = temp11.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_bind_path_is_not_name.
    FIELD-SYMBOLS <temp126> LIKE LINE OF mo_action->ms_actual-t_model_skipped.
    ...
    temp127 = sy-tabix.
    READ TABLE mo_action->ms_actual-t_model_skipped INDEX 1 ASSIGNING <temp126>.
    sy-tabix = temp127.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
