...
DATA lt_row TYPE temp3.
  ...
  METHOD test_message_box_dependent.
    ...
    FIELD-SYMBOLS <temp9> LIKE LINE OF mo_action->ms_next-s_action-t_custom.
    ...
    temp10 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 1 ASSIGNING <temp9>.
    sy-tabix = temp10.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_set_nav_routing.
    ...
    CLEAR temp13.
    INSERT z2ui5_if_client=>cs_nav_mode-fresh INTO TABLE temp13.
  ...
  METHOD test_set_nav_routing_lower.
    ...
    CLEAR temp15.
    INSERT `fresh` INTO TABLE temp15.
    li_client->follow_up_action( val   = z2ui5_if_client=>cs_event-set_nav_routing
                                 t_arg = temp15 ).
  ...
  METHOD test_hash_attach_changed.
    ...
    CLEAR temp17.
    INSERT `HASH_CHANGED` INTO TABLE temp17.
  ...
  METHOD test_hash_replace.
    ...
    CLEAR temp19.
    INSERT `/other` INTO TABLE temp19.
  ...
  METHOD test_follow_up_action_ev.
    ...
    CLEAR temp21.
    INSERT `My Title` INTO TABLE temp21.
    ...
    temp24 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 1 ASSIGNING <temp23>.
    sy-tabix = temp24.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp26 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 2 ASSIGNING <temp25>.
    sy-tabix = temp26.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_follow_up_action_nav.
    ...
    CLEAR temp27.
    INSERT `myContainer` INTO TABLE temp27.
    INSERT `myPage` INTO TABLE temp27.
    li_client->follow_up_action( val   = z2ui5_if_client=>cs_event-nav_container_to
                                 t_arg = temp27 ).

    CLEAR temp29.
    INSERT `popContainer` INTO TABLE temp29.
    INSERT `popPage` INTO TABLE temp29.
  ...
  METHOD test_follow_up_action_ctrl.
    ...
    FIELD-SYMBOLS <temp41> LIKE LINE OF mo_action->ms_next-s_action-t_custom.
    ...
    INSERT `Hello` INTO TABLE temp35.
    li_client->follow_up_action( val   = z2ui5_if_client=>cs_event-control_global
                                 t_arg = temp35 ).

    CLEAR temp37.
    INSERT `demoPanel` INTO TABLE temp37.
    INSERT `setExpanded` INTO TABLE temp37.
    INSERT `X` INTO TABLE temp37.
    ...
    temp42 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 1 ASSIGNING <temp41>.
    sy-tabix = temp42.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
        ...
        act = <temp41>-o_json->stringify( ) ).


    temp44 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 2 ASSIGNING <temp43>.
    sy-tabix = temp44.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp46 = sy-tabix.
    READ TABLE mo_action->ms_next-s_action-t_custom INDEX 3 ASSIGNING <temp45>.
    sy-tabix = temp46.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_event_arg_shorthand.
    ...
    CLEAR temp47.
    INSERT `${AUTHOR}` INTO TABLE temp47.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD test_event_arg_appends.
    ...
    CLEAR temp49.
    INSERT `first` INTO TABLE temp49.
    INSERT `second` INTO TABLE temp49.
  ...
  METHOD test_get_comp_params_memo.
    ...
    DATA li_client TYPE REF TO z2ui5_if_client.
    DATA temp53 TYPE REF TO z2ui5_if_ajson.
    DATA ls_get_1 TYPE z2ui5_if_client=>ty_s_get.
    DATA temp54 TYPE z2ui5_if_client=>ty_s_name_value-v.
    DATA temp55 TYPE z2ui5_if_client=>ty_s_name_value.
    DATA temp56 TYPE z2ui5_if_client=>ty_s_name_value-v.
    DATA temp57 TYPE z2ui5_if_client=>ty_s_name_value.
    DATA ls_get_2 TYPE z2ui5_if_client=>ty_s_get.
    li_client ?= mo_client.

    " FLP component data: one array per parameter name, first entry counts

    temp53 ?= z2ui5_cl_ajson=>parse( `{"startupParameters":{"foo":["bar"],"qty":["7","8"]}}` ).
    ...
    READ TABLE ls_get_1-t_comp_params INTO temp55 WITH KEY n = `foo`.
    IF sy-subrc = 0.
      temp54 = temp55-v.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
    ...
    READ TABLE ls_get_1-t_comp_params INTO temp57 WITH KEY n = `qty`.
    IF sy-subrc = 0.
      temp56 = temp57-v.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD test_omit_initial_paths.
    ...
    DATA temp60 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp61 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp62 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp63 TYPE z2ui5_if_ajson_types=>ty_node.
    DATA temp64 TYPE z2ui5_if_ajson_types=>ty_node.
    CLEAR temp58.
    INSERT `MIN` INTO TABLE temp58.
    INSERT `/ROWS/MAX` INTO TABLE temp58.

    CREATE OBJECT temp3 TYPE lcl_initial_paths_filter EXPORTING IT_PATHS = temp58.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_false
        act = li_filter->keep_node( temp60 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_false
        act = li_filter->keep_node( temp61 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_true
        act = li_filter->keep_node( temp62 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_true
        act = li_filter->keep_node( temp63 ) ).
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = abap_true
        act = li_filter->keep_node( is_node  = temp64
  ...
  METHOD test_omit_initial_decimals.
    ...
    DATA temp65 TYPE ty_s_row.
    DATA ls_row LIKE temp65.
    DATA temp66 TYPE REF TO z2ui5_if_ajson.
    DATA lo_ajson LIKE temp66.
    DATA temp67 TYPE REF TO lcl_empty_filter_keep_rows.
    DATA temp68 TYPE REF TO z2ui5_if_ajson.
    DATA temp69 TYPE REF TO lcl_empty_filter_keep_rows.
    CLEAR temp65.
    temp65-name = `A`.
    ...
    temp66 ?= z2ui5_cl_ajson=>create_empty( ).
    ...
    CREATE OBJECT temp67 TYPE lcl_empty_filter_keep_rows.
    cl_abap_unit_assert=>assert_equals(
        exp = `{"row":{"name":"A"}}`
        act = lo_ajson->filter( temp67 )->stringify( ) ).

    " a non-zero value in the same spelling survives
    ls_row-price = '0.01'.

    temp68 ?= z2ui5_cl_ajson=>create_empty( ).
    lo_ajson = temp68.
    lo_ajson->set( iv_ignore_empty = abap_false
                   iv_path         = `/row`
                   iv_val          = ls_row ).
    ...
    CREATE OBJECT temp69 TYPE lcl_empty_filter_keep_rows.
    cl_abap_unit_assert=>assert_equals(
        exp = `{"row":{"name":"A","price":0.01}}`
        act = lo_ajson->filter( temp69 )->stringify( ) ).
  ...
  METHOD test_omit_initial_keeps_rows.
    ...
    DATA temp70 TYPE ty_s_row.
    DATA temp71 TYPE ty_s_row.
    DATA temp72 TYPE REF TO z2ui5_if_ajson.
    DATA lo_ajson LIKE temp72.
    DATA lo_act TYPE REF TO z2ui5_if_ajson.
    DATA temp4 TYPE REF TO lcl_empty_filter_keep_rows.
    DATA temp73 TYPE ty_s_row.
    DATA ls_nest LIKE temp73.
    DATA temp74 TYPE REF TO z2ui5_if_ajson.
    DATA temp75 TYPE REF TO lcl_empty_filter_keep_rows.
    CLEAR temp70.
    temp70-title = `first`.
    temp70-count = 1.
    APPEND temp70 TO lt_tab.
    APPEND INITIAL LINE TO lt_tab.

    CLEAR temp71.
    temp71-title = `third`.
    temp71-count = 3.
    APPEND temp71 TO lt_tab.


    temp72 ?= z2ui5_cl_ajson=>create_empty( ).
    ...
    CREATE OBJECT temp75 TYPE lcl_empty_filter_keep_rows.
    cl_abap_unit_assert=>assert_equals(
        exp = ``
        act = lo_ajson->filter( temp75 )->stringify( ) ).
  ...
  METHOD test_omit_filters_serial.
    ...
    DATA temp76 TYPE string_table.
    DATA temp78 TYPE REF TO lcl_and_filter.
    DATA temp79 TYPE REF TO ltcl_bad_filter.
    ...
    CLEAR temp76.
    INSERT `MIN` INTO TABLE temp76.
    CREATE OBJECT li_paths TYPE lcl_initial_paths_filter EXPORTING IT_PATHS = temp76.
    cl_abap_unit_assert=>assert_true(
        z2ui5_cl_ui5_util_context=>rtti_check_serializable( li_paths ) ).


    CREATE OBJECT temp78 TYPE lcl_and_filter EXPORTING ii_first = li_paths ii_second = li_omit.
    cl_abap_unit_assert=>assert_true(
    ...
    CREATE OBJECT temp79 TYPE ltcl_bad_filter.
    cl_abap_unit_assert=>assert_false(
        z2ui5_cl_ui5_util_context=>rtti_check_serializable( temp79 ) ).
  ...
  METHOD test_bind_filter_not_serial.
    ...
    DATA lx TYPE REF TO z2ui5_cx_ui5_util_error.
        DATA temp80 TYPE REF TO ltcl_bad_filter.
    ...
    TRY.
        ...
        CREATE OBJECT temp80 TYPE ltcl_bad_filter.
        li_client->_bind( val           = lo_app->mv_name
                          custom_filter = temp80 ).
  ...
  METHOD test_omit_initial_db_save.
    ...
    FIELD-SYMBOLS <temp81> TYPE z2ui5_if_ui5_types=>ty_s_attri.
    ...
    READ TABLE lo_cont_db->mt_attri->* WITH KEY name = `MV_NAME` ASSIGNING <temp81>.
IF sy-subrc <> 0.
  ASSERT 1 = 0.
ENDIF.
GET REFERENCE OF <temp81> INTO lr_attri.
  ...
  METHOD test_bind_tab_cell.
    ...
    DATA li_client TYPE REF TO z2ui5_if_client.
    DATA lo_app TYPE REF TO ltcl_test_app.
    DATA temp82 TYPE ltcl_test_app=>ty_s_emp.
    DATA temp83 TYPE ltcl_test_app=>ty_s_emp.
    FIELD-SYMBOLS <temp84> LIKE LINE OF lo_app->mt_emp.
    DATA temp85 LIKE sy-tabix.
    FIELD-SYMBOLS <temp86> LIKE LINE OF lo_app->mt_emp.
    ...
    CLEAR temp82.
    temp82-name = `Michael Adams`.
    temp82-job = `Scrum Master`.
    INSERT temp82 INTO TABLE lo_app->mt_emp.

    CLEAR temp83.
    temp83-name = `John Miller`.
    temp83-job = `Product Owner`.
    INSERT temp83 INTO TABLE lo_app->mt_emp.



    temp85 = sy-tabix.
    READ TABLE lo_app->mt_emp INDEX 1 ASSIGNING <temp84>.
    sy-tabix = temp85.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
        exp = `{/MT_EMP/0/NAME}`
        act = li_client->_bind( val       = <temp84>-name
                                tab       = lo_app->mt_emp
                                tab_index = 1 ) ).
    ...
    temp87 = sy-tabix.
    READ TABLE lo_app->mt_emp INDEX 2 ASSIGNING <temp86>.
    sy-tabix = temp87.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals(
        exp = `{/MT_EMP/1/JOB}`
        act = li_client->_bind( val       = <temp86>-job
                                tab       = lo_app->mt_emp
                                tab_index = 2 ) ).
  ...
  METHOD test_bind_tab_cell_assign.
    ...
    DATA temp88 TYPE ltcl_test_app=>ty_s_emp.
    DATA temp89 TYPE ltcl_test_app=>ty_s_emp.

    li_client ?= mo_client.
    lo_app ?= mo_action->mo_app->mo_app.

    CLEAR temp88.
    temp88-name = `Michael Adams`.
    temp88-job = `Scrum Master`.
    INSERT temp88 INTO TABLE lo_app->mt_emp.

    CLEAR temp89.
    temp89-name = `John Miller`.
    temp89-job = `Product Owner`.
    INSERT temp89 INTO TABLE lo_app->mt_emp.
...
CLASS ltcl_test_model_skipped IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    DATA lo_http TYPE REF TO z2ui5_cl_ui5_handler.
    DATA temp90 TYPE ltcl_app_price_editor=>ty_s_pos.
    ...
    CLEAR temp90.
    temp90-qty = 1.
    APPEND temp90 TO ls_product-t_pos.
  ...
  METHOD roundtrip.
    ...
    IF model IS NOT INITIAL.
      ...
      temp91 ?= z2ui5_cl_ajson=>parse( model ).
  ...
  METHOD test_accepted_price_silent.
    ...
    ENDIF.
    temp5 = <temp1>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp92
                                        act = temp5 ).
  ...
  METHOD test_refused_scalar_reported.
    ...
    FIELD-SYMBOLS <temp93> LIKE LINE OF lt_skipped.
    DATA temp94 LIKE sy-tabix.
    FIELD-SYMBOLS <temp95> LIKE LINE OF lt_skipped.
    DATA temp96 LIKE sy-tabix.
    FIELD-SYMBOLS <temp97> LIKE LINE OF lt_skipped.
    DATA temp98 LIKE sy-tabix.
    DATA temp99 TYPE decfloat34.
    ...
    temp94 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp93>.
    sy-tabix = temp94.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp96 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp95>.
    sy-tabix = temp96.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp98 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp97>.
    sy-tabix = temp98.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    cl_abap_unit_assert=>assert_equals( exp = temp99
                                        act = temp6 ).

  ENDMETHOD.

  METHOD test_refused_price_reported.
    DATA temp100 TYPE decfloat34.
    ...
    ENDIF.
    temp7 = <temp3>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp100
                                        act = temp7 ).
  ...
  METHOD test_save_no_longer_lies.
    ...
    ENDIF.
    temp9 = <temp7>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp102
                                        act = temp9 ).
  ...
  METHOD test_nested_row_unresolved.
    FIELD-SYMBOLS <temp103> LIKE LINE OF mo_app->mt_product.
    DATA temp104 LIKE sy-tabix.
    FIELD-SYMBOLS <temp10> LIKE LINE OF <temp103>-t_pos.
    ...
    temp104 = sy-tabix.
    READ TABLE mo_app->mt_product INDEX 1 ASSIGNING <temp103>.
    sy-tabix = temp104.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp11 = sy-tabix.
    READ TABLE <temp103>-t_pos INDEX 1 ASSIGNING <temp10>.
    sy-tabix = temp11.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_bind_path_is_not_name.
    FIELD-SYMBOLS <temp105> LIKE LINE OF mo_action->ms_actual-t_model_skipped.
    ...
    temp106 = sy-tabix.
    READ TABLE mo_action->ms_actual-t_model_skipped INDEX 1 ASSIGNING <temp105>.
    sy-tabix = temp106.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
