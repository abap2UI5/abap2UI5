...
bind( temp140 ).
  ...
  METHOD delta_malformed_survives.
    ...
    DATA temp153 LIKE REF TO mo_app->mt_std.
    DATA temp154 TYPE string_table.
    DATA lt_hostile LIKE temp154.
    DATA lv_json LIKE LINE OF lt_hostile.
      FIELD-SYMBOLS <temp156> LIKE LINE OF mo_app->mt_std.
      DATA temp157 LIKE sy-tabix.
      FIELD-SYMBOLS <temp158> LIKE LINE OF mo_app->mt_std.
      DATA temp159 LIKE sy-tabix.
    FIELD-SYMBOLS <temp160> LIKE LINE OF mo_app->mt_std.
    DATA temp161 LIKE sy-tabix.
    FIELD-SYMBOLS <temp162> LIKE LINE OF mo_app->mt_std.
    DATA temp163 LIKE sy-tabix.
    GET REFERENCE OF mo_app->mt_std INTO temp153.
bind( temp153 ).


    CLEAR temp154.
    INSERT `{"MT_STD":{"__delta":"boom"}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":42}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":null}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":["a","b"]}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":"boom"}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":[1,2]}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":null}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"999999999999":{"COL1":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"1e3":{"COL1":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"":"boom"}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"":{"COL1":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":{"":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":{"NOPE":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":{"__DELTA":"Z"}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":{"COL1":{"__delta":{"0":{"X":1}}}}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":{"__delta":{"0":{"COL2":{"__delta":{"0":{"X":1}}}}}}}` INTO TABLE temp154.
    INSERT `{"MT_STD":"boom"}` INTO TABLE temp154.
    INSERT `{"MT_STD":42}` INTO TABLE temp154.
    ...
    LOOP AT lt_hostile INTO lv_json.
      ...
      temp157 = sy-tabix.
      READ TABLE mo_app->mt_std INDEX 1 ASSIGNING <temp156>.
      sy-tabix = temp157.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      ...
      temp159 = sy-tabix.
      READ TABLE mo_app->mt_std INDEX 2 ASSIGNING <temp158>.
      sy-tabix = temp159.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
    ...
    temp161 = sy-tabix.
    READ TABLE mo_app->mt_std INDEX 1 ASSIGNING <temp160>.
    sy-tabix = temp161.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp163 = sy-tabix.
    READ TABLE mo_app->mt_std INDEX 2 ASSIGNING <temp162>.
    sy-tabix = temp163.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_into_helper_table.
    ...
    DATA temp164 LIKE REF TO mo_app->mo_inner->mt_own.
    FIELD-SYMBOLS <temp165> LIKE LINE OF mo_app->mo_inner->mt_own.
    DATA temp166 LIKE sy-tabix.
    FIELD-SYMBOLS <temp167> LIKE LINE OF mo_app->mo_inner->mt_own.
    DATA temp168 LIKE sy-tabix.
    GET REFERENCE OF mo_app->mo_inner->mt_own INTO temp164.
bind( temp164 ).
    ...
    temp166 = sy-tabix.
    READ TABLE mo_app->mo_inner->mt_own INDEX 1 ASSIGNING <temp165>.
    sy-tabix = temp166.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp168 = sy-tabix.
    READ TABLE mo_app->mo_inner->mt_own INDEX 1 ASSIGNING <temp167>.
    sy-tabix = temp168.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_nested.
    ...
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_t_attri.
    DATA lo_model TYPE REF TO z2ui5_cl_ui5_srv_model.
    FIELD-SYMBOLS <temp169> LIKE LINE OF lo_app->mt_tree.
    DATA temp170 LIKE sy-tabix.
    FIELD-SYMBOLS <temp171> LIKE LINE OF lo_app->mt_tree.
    DATA temp172 LIKE sy-tabix.
    FIELD-SYMBOLS <temp34> LIKE LINE OF <temp171>-nodes.
    DATA temp35 LIKE sy-tabix.
    FIELD-SYMBOLS <temp173> LIKE LINE OF lo_app->mt_tree.
    DATA temp174 LIKE sy-tabix.
    FIELD-SYMBOLS <temp36> LIKE LINE OF <temp173>-nodes.
    DATA temp37 LIKE sy-tabix.
    FIELD-SYMBOLS <temp175> LIKE LINE OF lo_app->mt_tree.
    DATA temp176 LIKE sy-tabix.
    FIELD-SYMBOLS <temp177> LIKE LINE OF lo_app->mt_tree.
    DATA temp178 LIKE sy-tabix.
    FIELD-SYMBOLS <temp179> LIKE LINE OF lo_app->mt_tree.
    DATA temp180 LIKE sy-tabix.
    FIELD-SYMBOLS <temp181> LIKE LINE OF lo_app->mt_tree.
    DATA temp182 LIKE sy-tabix.
    FIELD-SYMBOLS <temp38> LIKE LINE OF <temp181>-nodes.
    ...
    temp170 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp169>.
    sy-tabix = temp170.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp172 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp171>.
    sy-tabix = temp172.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp35 = sy-tabix.
    READ TABLE <temp171>-nodes INDEX 2 ASSIGNING <temp34>.
    sy-tabix = temp35.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp174 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp173>.
    sy-tabix = temp174.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp37 = sy-tabix.
    READ TABLE <temp173>-nodes INDEX 1 ASSIGNING <temp36>.
    sy-tabix = temp37.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp176 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp175>.
    sy-tabix = temp176.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp178 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp177>.
    sy-tabix = temp178.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `Berlin`
                                        act = <temp177>-s_adr-city ).
    ...
    temp180 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp179>.
    sy-tabix = temp180.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 1
                                        act = lines( <temp179>-nodes ) ).


    temp182 = sy-tabix.
    READ TABLE lo_app->mt_tree INDEX 1 ASSIGNING <temp181>.
    sy-tabix = temp182.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp39 = sy-tabix.
    READ TABLE <temp181>-nodes INDEX 1 ASSIGNING <temp38>.
    sy-tabix = temp39.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_typed_cells.
    ...
    FIELD-SYMBOLS <temp186> LIKE LINE OF lo_app->mt_tab.
    DATA temp187 LIKE sy-tabix.
    FIELD-SYMBOLS <temp188> LIKE LINE OF lo_app->mt_tab.
    DATA temp189 LIKE sy-tabix.
    FIELD-SYMBOLS <temp190> LIKE LINE OF lo_app->mt_tab.
    DATA temp191 LIKE sy-tabix.
    FIELD-SYMBOLS <temp192> LIKE LINE OF lo_app->mt_tab.
    DATA temp193 LIKE sy-tabix.
    DATA temp194 TYPE decfloat34.
    ...
    temp187 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 1 ASSIGNING <temp186>.
    sy-tabix = temp187.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp189 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 1 ASSIGNING <temp188>.
    sy-tabix = temp189.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp191 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 2 ASSIGNING <temp190>.
    sy-tabix = temp191.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp193 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 2 ASSIGNING <temp192>.
    sy-tabix = temp193.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_initial( <temp192>-tm ).
    cl_abap_unit_assert=>assert_initial( lo_model->mt_skipped ).
    ...
    ENDIF.
    temp41 = <temp3>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp194
                                        act = temp41 ).


    temp196 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 1 ASSIGNING <temp195>.
    sy-tabix = temp196.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    ENDIF.
    temp42 = <temp5>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp197
                                        act = temp42 ).
    cl_abap_unit_assert=>assert_equals( exp = 2
  ...
  METHOD delta_trace.
    ...
    FIELD-SYMBOLS <temp198> LIKE LINE OF lo_app->mt_tab.
    DATA temp199 LIKE sy-tabix.
    FIELD-SYMBOLS <temp45> LIKE LINE OF <temp198>-t_pos.
    DATA temp46 LIKE sy-tabix.
    FIELD-SYMBOLS <temp200> LIKE LINE OF lo_model->mt_skipped.
    ...
    temp199 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 2 ASSIGNING <temp198>.
    sy-tabix = temp199.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp46 = sy-tabix.
    READ TABLE <temp198>-t_pos INDEX 1 ASSIGNING <temp45>.
    sy-tabix = temp46.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp201 = sy-tabix.
    READ TABLE lo_model->mt_skipped INDEX 1 ASSIGNING <temp200>.
    sy-tabix = temp201.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_sorted_refused.
    ...
    DATA lo_app TYPE REF TO ltcl_app_typed.
    DATA temp202 TYPE ltcl_app_typed=>ty_s_row.
    DATA lo_model TYPE REF TO z2ui5_cl_ui5_srv_model.
    DATA temp203 TYPE decfloat34.
    DATA temp47 TYPE decfloat34.
    FIELD-SYMBOLS <temp7> LIKE LINE OF lo_app->mt_sorted.
    DATA temp8 LIKE sy-tabix.
    FIELD-SYMBOLS <temp204> LIKE LINE OF lo_model->mt_skipped.
    DATA temp205 LIKE sy-tabix.
    DATA temp37 TYPE xsdboolean.
    DATA temp6 LIKE sy-subrc.
    DATA temp206 LIKE REF TO mo_app->mt_sorted.
    FIELD-SYMBOLS <temp207> LIKE LINE OF mo_app->mt_sorted.
    DATA temp208 LIKE sy-tabix.
    lo_app = typed_app( ).

    CLEAR temp202.
    temp202-name = `Monitor`.
    temp202-price = '299.00'.
    INSERT temp202 INTO TABLE lo_app->mt_sorted.
    ...
    ENDIF.
    temp47 = <temp7>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp203
                                        act = temp47 ).
    cl_abap_unit_assert=>assert_equals( exp = 2
                                        act = lines( lo_model->mt_skipped ) ).
    ...
    temp205 = sy-tabix.
    READ TABLE lo_model->mt_skipped INDEX 1 ASSIGNING <temp204>.
    sy-tabix = temp205.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    GET REFERENCE OF mo_app->mt_sorted INTO temp206.
bind( temp206 ).
    mo_model->main_json_to_attri( delta( `{"MT_SORTED":{"__delta":{"0":{"COL2":1}}}}` ) ).


    temp208 = sy-tabix.
    READ TABLE mo_app->mt_sorted INDEX 1 ASSIGNING <temp207>.
    sy-tabix = temp208.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_kind_refused.
    ...
    DATA temp209 LIKE REF TO mo_app->mt_rows_ref.
    DATA lr_before TYPE REF TO string.
    FIELD-SYMBOLS <temp48> LIKE LINE OF mo_app->mt_rows_ref.
    DATA temp49 LIKE sy-tabix.
    FIELD-SYMBOLS <temp210> LIKE LINE OF mo_model->mt_skipped.
    DATA temp211 LIKE sy-tabix.
    FIELD-SYMBOLS <temp212> LIKE LINE OF mo_model->mt_skipped.
    DATA temp213 LIKE sy-tabix.
    FIELD-SYMBOLS <temp214> LIKE LINE OF mo_app->mt_rows_ref.
    DATA temp215 LIKE sy-tabix.
    DATA temp38 TYPE xsdboolean.
    FIELD-SYMBOLS <temp216> LIKE LINE OF mo_app->mt_rows_ref.
    DATA temp217 LIKE sy-tabix.
    DATA temp218 LIKE REF TO mo_app->mt_nested.
    FIELD-SYMBOLS <temp219> LIKE LINE OF mo_model->mt_skipped.
    DATA temp220 LIKE sy-tabix.
    FIELD-SYMBOLS <temp221> LIKE LINE OF mo_app->mt_nested.
    DATA temp222 LIKE sy-tabix.
    FIELD-SYMBOLS <temp223> LIKE LINE OF mo_app->mt_nested.
    DATA temp224 LIKE sy-tabix.
    GET REFERENCE OF mo_app->mt_rows_ref INTO temp209.
bind( temp209 ).
    ...
    temp211 = sy-tabix.
    READ TABLE mo_model->mt_skipped INDEX 1 ASSIGNING <temp210>.
    sy-tabix = temp211.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp213 = sy-tabix.
    READ TABLE mo_model->mt_skipped INDEX 1 ASSIGNING <temp212>.
    sy-tabix = temp213.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp215 = sy-tabix.
    READ TABLE mo_app->mt_rows_ref INDEX 1 ASSIGNING <temp214>.
    sy-tabix = temp215.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.

    temp38 = boolc( <temp214>-r_elem = lr_before ).
    cl_abap_unit_assert=>assert_true( temp38 ).


    temp217 = sy-tabix.
    READ TABLE mo_app->mt_rows_ref INDEX 1 ASSIGNING <temp216>.
    sy-tabix = temp217.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    GET REFERENCE OF mo_app->mt_nested INTO temp218.
bind( temp218 ).
    ...
    temp220 = sy-tabix.
    READ TABLE mo_model->mt_skipped INDEX 1 ASSIGNING <temp219>.
    sy-tabix = temp220.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp222 = sy-tabix.
    READ TABLE mo_app->mt_nested INDEX 1 ASSIGNING <temp221>.
    sy-tabix = temp222.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 1
                                        act = lines( <temp221>-t_items ) ).


    temp224 = sy-tabix.
    READ TABLE mo_app->mt_nested INDEX 1 ASSIGNING <temp223>.
    sy-tabix = temp224.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD delta_mass_edit.
    ...
    DATA lo_app TYPE REF TO ltcl_app_typed.
      DATA temp225 TYPE ltcl_app_typed=>ty_s_row.
    ...
    DATA temp229 TYPE decfloat34.
    DATA temp55 TYPE decfloat34.
    FIELD-SYMBOLS <temp11> LIKE LINE OF lo_app->mt_tab.
    DATA temp12 LIKE sy-tabix.
    DATA temp230 TYPE decfloat34.
    DATA temp56 TYPE decfloat34.
    FIELD-SYMBOLS <temp13> LIKE LINE OF lo_app->mt_tab.
    DATA temp14 LIKE sy-tabix.
    FIELD-SYMBOLS <temp231> LIKE LINE OF lo_app->mt_tab.
    DATA temp232 LIKE sy-tabix.
    FIELD-SYMBOLS <temp57> LIKE LINE OF <temp231>-t_pos.
    DATA temp58 LIKE sy-tabix.
    FIELD-SYMBOLS <temp233> LIKE LINE OF lo_app->mt_tab.
    DATA temp234 LIKE sy-tabix.
    FIELD-SYMBOLS <temp59> LIKE LINE OF <temp233>-t_sorted.
    DATA temp60 LIKE sy-tabix.
    FIELD-SYMBOLS <temp235> LIKE LINE OF lo_app->mt_tab.
    DATA temp236 LIKE sy-tabix.
    FIELD-SYMBOLS <temp61> LIKE LINE OF <temp235>-t_pos.
    ...
    DO 6 TIMES.
      ...
      CLEAR temp225.
      temp225-name = |row-{ sy-index }|.
      temp225-price = sy-index * 100.

      CLEAR temp50.

      temp51-qty = sy-index.
      INSERT temp51 INTO TABLE temp50.
      temp225-t_pos = temp50.

      CLEAR temp52.

      temp53-qty = 1.
      INSERT temp53 INTO TABLE temp52.
      temp225-t_sorted = temp52.
      APPEND temp225 TO lo_app->mt_tab.
    ENDDO.
    ...
    DO 6 TIMES.
      ...
      temp227 = sy-tabix.
      READ TABLE lo_app->mt_tab INDEX sy-index ASSIGNING <temp226>.
      sy-tabix = temp227.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
    ...
    ENDIF.
    temp55 = <temp11>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp229
                                        act = temp55 ).
    ...
    ENDIF.
    temp56 = <temp13>-price.
    cl_abap_unit_assert=>assert_equals( exp = temp230
                                        act = temp56 ).
    " the nested standard table took its cell, the nested sorted one did not


    temp232 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 4 ASSIGNING <temp231>.
    sy-tabix = temp232.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp58 = sy-tabix.
    READ TABLE <temp231>-t_pos INDEX 1 ASSIGNING <temp57>.
    sy-tabix = temp58.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp234 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 5 ASSIGNING <temp233>.
    sy-tabix = temp234.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp60 = sy-tabix.
    READ TABLE <temp233>-t_sorted INDEX 1 ASSIGNING <temp59>.
    sy-tabix = temp60.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp236 = sy-tabix.
    READ TABLE lo_app->mt_tab INDEX 2 ASSIGNING <temp235>.
    sy-tabix = temp236.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp62 = sy-tabix.
    READ TABLE <temp235>-t_pos INDEX 1 ASSIGNING <temp61>.
    sy-tabix = temp62.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
...
CLASS ltcl_05_draft IMPLEMENTATION.
  ...
  METHOD roundtrip_new_instance.
    ...
    DATA lv_before TYPE string.
    FIELD-SYMBOLS <temp237> LIKE LINE OF mo_app->mt_rows_ref.
    DATA temp238 LIKE sy-tabix.
    FIELD-SYMBOLS <temp239> LIKE LINE OF mo_app->mt_rows_ref.
    DATA temp240 LIKE sy-tabix.
    FIELD-SYMBOLS <temp241> LIKE LINE OF mo_app->mt_comp.
    ...
    temp238 = sy-tabix.
    READ TABLE mo_app->mt_rows_ref INDEX 1 ASSIGNING <temp237>.
    sy-tabix = temp238.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp240 = sy-tabix.
    READ TABLE mo_app->mt_rows_ref INDEX 1 ASSIGNING <temp239>.
    sy-tabix = temp240.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `cell-obj`
                                        act = <temp239>-o_obj->mv_inner ).
    ...
    temp242 = sy-tabix.
    READ TABLE mo_app->mt_comp INDEX 1 ASSIGNING <temp241>.
    sy-tabix = temp242.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_not_bound( <temp241>-type ).
  ...
  METHOD payload_on_canonical_row.
    ...
    DATA temp243 TYPE REF TO cl_abap_tabledescr.
    DATA lo_tab LIKE temp243.
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_t_attri.
    DATA lo_model TYPE REF TO z2ui5_cl_ui5_srv_model.
    ...
    FIELD-SYMBOLS <temp246> LIKE LINE OF lr_attri->*.
    ...
    temp243 ?= cl_abap_typedescr=>describe_by_data( lo_app->mz_inner->mt_own ).
    ...
    temp245 = sy-tabix.
    READ TABLE lr_attri->* WITH KEY name = `MZ_INNER->MR_SHARED` ASSIGNING <temp244>.
    sy-tabix = temp245.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_not_initial( <temp244>-srtti_data ).


    temp247 = sy-tabix.
    READ TABLE lr_attri->* WITH KEY name = `MR_TABLE` ASSIGNING <temp246>.
    sy-tabix = temp247.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_initial( <temp246>-srtti_data ).
...
lr_attri = mo_model->main_attri_search( temp248 ).
    cl_abap_unit_assert=>assert_equals( exp = `MV_STRING`
                                        act = lr_attri->name ).
  ...
  METHOD broken_payload_unbound_ok.
    ...
    DATA temp249 LIKE REF TO mo_app->mv_string.
    DATA lr_broken TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    GET REFERENCE OF mo_app->mv_string INTO temp249.
bind( temp249 ).
    mo_model->main_attri_db_save_srtti( ).
  ...
  METHOD attribute_gone_skipped.
    ...
    DATA lv_app_xml TYPE string.
    DATA lv_attri_xml TYPE string.
    DATA temp250 TYPE z2ui5_if_ui5_types=>ty_s_attri.
    DATA temp251 TYPE z2ui5_if_ui5_types=>ty_s_attri.
    DATA temp252 TYPE z2ui5_if_ui5_types=>ty_s_attri.
    ...
    CLEAR temp250.
    temp250-name = `MV_GONE`.
    temp250-check_dissolved = abap_true.
    temp250-type_kind = row( `MV_STRING` )-type_kind.
    temp250-kind = row( `MV_STRING` )-kind.
    temp250-bind = abap_true.
    temp250-name_client = `/MV_GONE`.
    INSERT temp250 INTO TABLE mr_attri->*.

    CLEAR temp251.
    temp251-name = `MR_GONE`.
    temp251-check_dissolved = abap_true.
    temp251-type_kind = z2ui5_cl_ui5_util_context=>cv_typedescr_typekind_dref.
    temp251-srtti_data = `payload of a reference nobody has`.
    INSERT temp251 INTO TABLE mr_attri->*.

    CLEAR temp252.
    temp252-name = `MR_GONE->*`.
    temp252-name_parent = `MR_GONE`.
    temp252-check_dissolved = abap_true.
    temp252-type_kind = z2ui5_cl_ui5_util_context=>cv_typedescr_typekind_table.
    temp252-name_ref = `MR_SHARED_B->*`.
    INSERT temp252 INTO TABLE mr_attri->*.
  ...
  METHOD attribute_new_found.
    DATA lv_app_xml TYPE string.
    DATA lv_attri_xml TYPE string.
    DATA temp253 LIKE REF TO mo_app->mv_xstr.
DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    ...
    GET REFERENCE OF mo_app->mv_xstr INTO temp253.

lr_attri = mo_model->main_attri_search( temp253 ).
  ...
  METHOD class_swap_before_load.
    ...
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_t_attri.
    DATA lo_model TYPE REF TO z2ui5_cl_ui5_srv_model.
    DATA temp254 LIKE REF TO lo_host->mv_selectedkey.
DATA ls_bind TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    DATA lv_app_xml TYPE string.
    DATA lv_attri_xml TYPE string.
    ...
    DATA temp257 LIKE REF TO lo_host->mv_selectedkey.
    ...
    GET REFERENCE OF lo_host->mv_selectedkey INTO temp254.

ls_bind = lo_model->main_attri_search( temp254 ).
    ls_bind->bind = abap_true.
    ...
    temp256 = sy-tabix.
    READ TABLE lr_attri->* WITH KEY name = `MO_APP->MO_LAYOUT->MV_INNER` ASSIGNING <temp255>.
    sy-tabix = temp256.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_not_bound( <temp255>-o_typedescr ).

    " the host's own bind first - it walks the A rows of its own kind and
    " used to dump on the first one without a descriptor

    GET REFERENCE OF lo_host->mv_selectedkey INTO temp257.
ls_bind = lo_model->main_attri_search( temp257 ).
  ...
  METHOD bind_options_survive.
    DATA temp258 LIKE REF TO mo_app->ms_flat.
...
DATA lr_json TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    ...
    GET REFERENCE OF mo_app->ms_flat INTO temp258.

lr_flat = bind( temp258 ).
...
lr_json = bind( temp259 ).
  ...
  METHOD markup_survives.
    ...
    FIELD-SYMBOLS <row> TYPE any.
    DATA ls_sel TYPE ltcl_app_shapes=>ty_s_row_sel.
    DATA temp260 LIKE REF TO mo_app->mv_markup.
    DATA lv_before TYPE string.
    ...
    GET REFERENCE OF mo_app->mv_markup INTO temp260.
bind( temp260 ).
    bind( mo_app->mr_handle_tab ).
  ...
  METHOD cell_bind_after_restore.
    ...
    FIELD-SYMBOLS <temp261> TYPE ltcl_shp_inner=>ty_s_row.
DATA lr_row LIKE REF TO <temp261>.
    DATA temp262 LIKE REF TO mo_app->mo_inner->mt_own.
...
DATA temp64 TYPE z2ui5_if_ui5_types=>ty_s_bind_config.
    ...
    READ TABLE mo_app->mo_inner->mt_own INDEX 1 ASSIGNING <temp261>.
IF sy-subrc <> 0.
  ASSERT 1 = 0.
ENDIF.

GET REFERENCE OF <temp261> INTO lr_row.

    GET REFERENCE OF mo_app->mo_inner->mt_own INTO temp262.
...
GET REFERENCE OF <cell> INTO temp263.

CLEAR temp64.
temp64-tab = mo_app->mr_handle_tab.
temp64-tab_index = 2.
lv_path = lo_bind->main( val    = temp263
                             config = temp64 ).
  ...
  METHOD interface_and_obj_table.
    ...
    DATA lo_other TYPE REF TO ltcl_app_shapes.
    DATA temp264 LIKE REF TO lo_other->mv_string.
    DATA temp265 LIKE REF TO mo_app->mv_string.
    DATA lv_before TYPE string.
    DATA lo_restored TYPE REF TO ltcl_app_shapes.
    FIELD-SYMBOLS <temp266> LIKE LINE OF mo_app->mt_apps.
    DATA temp267 LIKE sy-tabix.
    CREATE OBJECT lo_other TYPE ltcl_app_shapes.
    lo_other->mv_string = `other`.
    mo_app->mi_app = lo_other.

    GET REFERENCE OF lo_other->mv_string INTO temp264.
bind( temp264 ).

    GET REFERENCE OF mo_app->mv_string INTO temp265.
bind( temp265 ).
    ...
    temp267 = sy-tabix.
    READ TABLE mo_app->mt_apps INDEX 1 ASSIGNING <temp266>.
    sy-tabix = temp267.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `in-table`
                                        act = <temp266>->mv_inner ).
    inv_json_unchanged( lv_before ).
    inv_search_finds_bound( ).
  ...
  METHOD alias_repointed_survives.
    ...
    DATA temp268 TYPE REF TO cl_abap_tabledescr.
    DATA lo_tab LIKE temp268.
    ...
    temp268 ?= z2ui5_cl_ui5_util_context=>rtti_get_typedescr_by_data_ref( mo_app->mr_shared_b ).
  ...
  METHOD live_recreated_same_type.
    ...
    DATA temp269 TYPE REF TO cl_abap_tabledescr.
    DATA lo_tab LIKE temp269.
    ...
    temp269 ?= z2ui5_cl_ui5_util_context=>rtti_get_typedescr_by_data_ref( mo_app->mr_shared_a ).
  ...
  METHOD save_without_main_is_noop.
    ...
    DATA temp270 LIKE LINE OF lt_rows.
    DATA lr_old LIKE REF TO temp270.
  ...
  METHOD late_alias_survives_save.
    ...
    DATA temp271 LIKE REF TO lo_late->mv_inner.
DATA lr_value TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    ...
    GET REFERENCE OF lo_late->mv_inner INTO temp271.

lr_value = bind( temp271 ).
...
CLASS ltcl_app_struct_alias IMPLEMENTATION.
  ...
  METHOD fill.
    DATA temp272 TYPE ltcl_app_struct_alias=>ty_t_row.
...
GET REFERENCE OF <tab> INTO temp274.

lr_row = lo_model->main_attri_search( temp274 ).
...
CLASS ltcl_app_two_refs IMPLEMENTATION.
  ...
  METHOD fill.
    DATA temp275 TYPE ltcl_app_two_refs=>ty_t_row.
...
CLASS ltcl_06_two_refs IMPLEMENTATION.
  ...
  METHOD both_refs_back_on_table.
    ...
    DATA lr_attri TYPE REF TO z2ui5_if_ui5_types=>ty_t_attri.
    DATA lo_model TYPE REF TO z2ui5_cl_ui5_srv_model.
    DATA temp277 TYPE REF TO data.
DATA lr_row TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    DATA temp278 LIKE LINE OF lr_attri->*.
    DATA lr_alias LIKE REF TO temp278.
    DATA lv_app_xml TYPE string.
    DATA lv_attri_xml TYPE string.
...
GET REFERENCE OF <tab> INTO temp277.

lr_row = lo_model->main_attri_search( temp277 ).
    lr_row->bind        = abap_true.
...
CLASS ltcl_app_view_host IMPLEMENTATION.
  ...
  METHOD create_sub_app.
    ...
    DATA temp279 TYPE ltcl_app_sub_view=>ty_t_row.
...
CLASS ltcl_07_view_host IMPLEMENTATION.
  ...
  METHOD row_exists.
    ...
    DATA temp281 LIKE sy-subrc.
    DATA temp69 TYPE xsdboolean.
    READ TABLE mr_attri->* WITH KEY name = iv_name TRANSPORTING NO FIELDS.
    temp281 = sy-subrc.

    temp69 = boolc( temp281 = 0 ).
    result = temp69.
  ...
  METHOD subapp_binds_under_host.
    DATA temp282 TYPE REF TO ltcl_app_sub_view.
    DATA lo_sub LIKE temp282.
    DATA temp283 LIKE REF TO lo_sub->mt_table.
DATA lr_row TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    DATA lv_json TYPE string.
    DATA temp70 TYPE xsdboolean.
    DATA temp284 TYPE REF TO z2ui5_if_ajson.
    DATA lo_front LIKE temp284.
    FIELD-SYMBOLS <temp285> LIKE LINE OF lo_sub->mt_table.
    ...
    temp282 ?= mo_host->mo_app.

    lo_sub = temp282.

    GET REFERENCE OF lo_sub->mt_table INTO temp283.

lr_row = mo_model->main_attri_search( temp283 ).
    cl_abap_unit_assert=>assert_equals( exp = `MO_APP->MT_TABLE`
                                        act = lr_row->name ).
    ...
    temp286 = sy-tabix.
    READ TABLE lo_sub->mt_table INDEX 1 ASSIGNING <temp285>.
    sy-tabix = temp286.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD subapp_created_late_found.
    ...
    DATA temp287 LIKE REF TO mo_host->mv_selectedkey.
DATA lr_key TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    FIELD-SYMBOLS <temp288> LIKE LINE OF mr_attri->*.
    DATA temp289 LIKE sy-tabix.
    GET REFERENCE OF mo_host->mv_selectedkey INTO temp287.

lr_key = mo_model->main_attri_search( temp287 ).
    ...
    temp289 = sy-tabix.
    READ TABLE mr_attri->* WITH KEY name = `MV_SELECTEDKEY` ASSIGNING <temp288>.
    sy-tabix = temp289.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD host_survives_roundtrip.
    DATA temp290 TYPE REF TO ltcl_app_sub_view.
    DATA lo_live LIKE temp290.
    DATA temp291 LIKE REF TO lo_live->mt_table.
DATA lr_row TYPE REF TO z2ui5_if_ui5_types=>ty_s_attri.
    DATA lv_before TYPE string.
    DATA lv_app_xml TYPE string.
    DATA lv_attri_xml TYPE string.
    DATA temp292 TYPE REF TO ltcl_app_sub_view.
    DATA lo_back LIKE temp292.

    mo_host->create_sub_app( ).

    temp290 ?= mo_host->mo_app.

    lo_live = temp290.

    GET REFERENCE OF lo_live->mt_table INTO temp291.

lr_row = mo_model->main_attri_search( temp291 ).
    lr_row->bind        = abap_true.
    lr_row->name_client = `/MT_TABLE`.
