...
CLASS ltcl_01_xml IMPLEMENTATION.
  ...
  METHOD line_feed_survives.
    ...
    lv_exp = |line one{ lv_nl }line two{ lv_nl }{ lv_nl }after a blank one|.
...
CLASS ltcl_03_errors IMPLEMENTATION.
  ...
  METHOD broken_payload_bound_loud.
    ...
    TRY.
      ...
      CATCH z2ui5_cx_ui5_util_error INTO lx.
        ...
        temp33 = boolc( lv_text CS `APP_STATE_RESTORE_ERROR` ).
  ...
  METHOD broken_payload_unbound_ok.
    ...
    DATA temp16 LIKE REF TO mo_user->mv_string.
    DATA lo_loaded TYPE REF TO z2ui5_cl_ui5_app_cont.
    DATA lo_app TYPE REF TO ltcl_cont_app.
    DATA temp34 TYPE xsdboolean.
    GET REFERENCE OF mo_user->mv_string INTO temp16.
bind( ir_val  = temp16
          iv_path = `/MV_STRING` ).
...
bind( ir_val  = temp17
  ...
  METHOD parse_writes_back.
    DATA lo_front TYPE REF TO z2ui5_cl_ajson.
  ...
  METHOD parse_answers_refused.
    ...
    DATA temp18 LIKE REF TO mo_user->mt_std.
    DATA lo_front TYPE REF TO z2ui5_cl_ajson.
    DATA lt_skipped TYPE z2ui5_if_client=>ty_t_model_skip.
    FIELD-SYMBOLS <temp19> LIKE LINE OF lt_skipped.
    DATA temp20 LIKE sy-tabix.
    FIELD-SYMBOLS <temp21> LIKE LINE OF lt_skipped.
    DATA temp22 LIKE sy-tabix.
    FIELD-SYMBOLS <temp23> LIKE LINE OF lt_skipped.
    DATA temp24 LIKE sy-tabix.
    FIELD-SYMBOLS <temp25> LIKE LINE OF mo_user->mt_std.
    DATA temp26 LIKE sy-tabix.
    FIELD-SYMBOLS <temp27> LIKE LINE OF mo_user->mt_std.
    DATA temp28 LIKE sy-tabix.
    GET REFERENCE OF mo_user->mt_std INTO temp18.
bind( ir_val  = temp18
          iv_path = `/MT_STD` ).
    ...
    temp20 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp19>.
    sy-tabix = temp20.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp22 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp21>.
    sy-tabix = temp22.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp24 = sy-tabix.
    READ TABLE lt_skipped INDEX 1 ASSIGNING <temp23>.
    sy-tabix = temp24.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp26 = sy-tabix.
    READ TABLE mo_user->mt_std INDEX 1 ASSIGNING <temp25>.
    sy-tabix = temp26.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp28 = sy-tabix.
    READ TABLE mo_user->mt_std INDEX 1 ASSIGNING <temp27>.
    sy-tabix = temp28.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
