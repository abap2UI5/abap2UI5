...
CLASS ltcl_test_client DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS test_toast_client         FOR TESTING RAISING cx_static_check.
    METHODS test_control_call         FOR TESTING RAISING cx_static_check.
    METHODS test_control_call_client  FOR TESTING RAISING cx_static_check.
    METHODS test_binding_filter       FOR TESTING RAISING cx_static_check.
    METHODS test_binding_filter_client FOR TESTING RAISING cx_static_check.
    METHODS test_binding_sort         FOR TESTING RAISING cx_static_check.
    METHODS test_binding_sort_client  FOR TESTING RAISING cx_static_check.
...
CLASS ltcl_test_client IMPLEMENTATION.
  ...
  METHOD test_toast_client.

    DATA li_client TYPE REF TO z2ui5_if_client.
    li_client ?= mo_client.

    " the whole contract of every named wrapper: the emitted handler/wire is
    " BYTE-IDENTICAL to the generic positional form it replaces
    cl_abap_unit_assert=>assert_equals(
        exp = li_client->_event_client(
                  val   = z2ui5_if_client=>cs_event-control_global
                  t_arg = VALUE #( ( `MESSAGE_TOAST` ) ( `show` )
                                   ( `Item selected: {0}` )
                                   ( `${$parameters>/item}.getText()` ) ) )
        act = li_client->toast_client(
                  template = `Item selected: {0}`
                  t_arg    = VALUE #( ( `${$parameters>/item}.getText()` ) ) ) ).

    " a toast without client-resolved args is just the template
    cl_abap_unit_assert=>assert_equals(
        exp = li_client->_event_client(
                  val   = z2ui5_if_client=>cs_event-control_global
                  t_arg = VALUE #( ( `MESSAGE_TOAST` ) ( `show` ) ( `Hello` ) ) )
        act = li_client->toast_client( `Hello` ) ).

  ENDMETHOD.


  METHOD test_control_call.

    DATA li_client TYPE REF TO z2ui5_if_client.
    li_client ?= mo_client.

    li_client->control_call( id     = `demoPanel`
                             method = `setExpanded`
                             t_arg  = VALUE #( ( `X` ) ) ).
    li_client->control_call( id     = `demoPanel`
                             method = `setExpanded`
  ...
  METHOD test_control_call_client.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = li_client->_event_client(
                  ...
                  t_arg = VALUE #( ( `dialog` ) ( `close` ) ) )
        act = li_client->control_call_client( id     = `dialog`
                                              method = `close`
                                              view   = z2ui5_if_client=>cs_view-popup ) ).
  ...
  METHOD test_binding_filter.

    DATA li_client TYPE REF TO z2ui5_if_client.
    li_client ?= mo_client.

    li_client->binding_filter( id       = `list1`
                               path     = `NAME`
                               operator = `Contains`
                               value1   = `abc` ).

    " trailing empty params (value2 here) are dropped by the serializer,
    " exactly as in the hand-written generic form
    cl_abap_unit_assert=>assert_equals(
        exp = `["BINDING_CALL","list1","items","filter","NAME","Contains","abc"]`
        act = mo_action->ms_next-s_set-s_follow_up_action-custom_js[ 1 ] ).

  ENDMETHOD.


  METHOD test_binding_filter_client.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = li_client->_event_client(
                  ...
                  t_arg = VALUE #( ( `list1` ) ( `items` ) ( `filter` )
                                   ( `NAME` ) ( `Contains` ) ( `${$parameters>/query}` ) ( `` ) ) )
        act = li_client->binding_filter_client( id       = `list1`
                                                path     = `NAME`
                                                operator = `Contains`
                                                value1   = `${$parameters>/query}` ) ).

  ENDMETHOD.


  METHOD test_binding_sort.

    DATA li_client TYPE REF TO z2ui5_if_client.
    li_client ?= mo_client.

    li_client->binding_sort( id         = `list1`
                             path       = `NAME`
                             descending = abap_true ).

    " descending travels as the wire's `X` token, the empty group flag is a
    " dropped trailing param - both exactly as in the generic form
    cl_abap_unit_assert=>assert_equals(
        exp = `["BINDING_CALL","list1","items","sort","NAME","X"]`
        act = mo_action->ms_next-s_set-s_follow_up_action-custom_js[ 1 ] ).

  ENDMETHOD.


  METHOD test_binding_sort_client.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = li_client->_event_client(
                  val   = z2ui5_if_client=>cs_event-binding_call
                  t_arg = VALUE #( ( `list1` ) ( `items` ) ( `sort` )
                                   ( `NAME` ) ( `X` ) ( `` ) ) )
        act = li_client->binding_sort_client( id         = `list1`
                                              path       = `NAME`
                                              descending = abap_true ) ).
