CLASS z2ui5_cl_ajson DEFINITION
  ...
  PUBLIC SECTION.
    ...
    ALIASES is_empty                   FOR z2ui5_if_ajson~is_empty.
    ALIASES exists                     FOR z2ui5_if_ajson~exists.
    ALIASES members                    FOR z2ui5_if_ajson~members.
    ALIASES get                        FOR z2ui5_if_ajson~get.
    ALIASES get_boolean                FOR z2ui5_if_ajson~get_boolean.
    ALIASES get_integer                FOR z2ui5_if_ajson~get_integer.
    ALIASES get_number                 FOR z2ui5_if_ajson~get_number.
    ALIASES get_date                   FOR z2ui5_if_ajson~get_date.
    ALIASES get_timestamp              FOR z2ui5_if_ajson~get_timestamp.
    ALIASES get_string                 FOR z2ui5_if_ajson~get_string.
    ALIASES slice                      FOR z2ui5_if_ajson~slice.
    ALIASES to_abap                    FOR z2ui5_if_ajson~to_abap.
    ALIASES array_to_string_table      FOR z2ui5_if_ajson~array_to_string_table.

    ALIASES clear                      FOR z2ui5_if_ajson~clear.
    ALIASES set                        FOR z2ui5_if_ajson~set.
    ALIASES setx                       FOR z2ui5_if_ajson~setx.
    ALIASES set_boolean                FOR z2ui5_if_ajson~set_boolean.
    ALIASES set_string                 FOR z2ui5_if_ajson~set_string.
    ALIASES set_integer                FOR z2ui5_if_ajson~set_integer.
    ALIASES set_date                   FOR z2ui5_if_ajson~set_date.
    ALIASES set_timestamp              FOR z2ui5_if_ajson~set_timestamp.
    ALIASES set_null                   FOR z2ui5_if_ajson~set_null.
    ALIASES delete                     FOR z2ui5_if_ajson~delete.
    ALIASES touch_array                FOR z2ui5_if_ajson~touch_array.
    ALIASES push                       FOR z2ui5_if_ajson~push.
    ALIASES stringify                  FOR z2ui5_if_ajson~stringify.

    ALIASES clone                      FOR z2ui5_if_ajson~clone.
    ALIASES filter                     FOR z2ui5_if_ajson~filter.
    ALIASES map                        FOR z2ui5_if_ajson~map.

    ALIASES mt_json_tree               FOR z2ui5_if_ajson~mt_json_tree.
    ALIASES keep_item_order            FOR z2ui5_if_ajson~keep_item_order.
    ALIASES format_datetime            FOR z2ui5_if_ajson~format_datetime.
    ALIASES to_abap_corresponding_only FOR z2ui5_if_ajson~to_abap_corresponding_only.
    ALIASES freeze                     FOR z2ui5_if_ajson~freeze.
...
CLASS z2ui5_cl_ajson IMPLEMENTATION.
  ...
  METHOD create_from.
    ...
    ELSE.
      ...
      ENDIF.
      lo_mutator_queue->lif_mutator_runner~run( EXPORTING it_source_tree = ii_source_json->mt_json_tree
                                                IMPORTING et_dest_tree   = ro_instance->mt_json_tree ).
    ENDIF.
  ...
  METHOD get_item.
    ...
    ASSIGN mt_json_tree[ path = ls_path_name-path
                         name = ls_path_name-name ] TO <item>.
    IF sy-subrc = 0.
  ...
  METHOD parse.
    ...
    ro_instance = NEW #( ).
    lo_parser = NEW #( ).
    ro_instance->mt_json_tree      = lo_parser->parse( iv_json            = iv_json
                                                       iv_keep_item_order = iv_keep_item_order ).
    ro_instance->mi_custom_mapping = ii_custom_mapping.
      ...
      IF sy-subrc <> 0. " New node, assume it is always object as it has a named child, use touch_array to init array
        ...
        IF lr_node_parent IS NOT INITIAL. " if has parent
          ...
          IF lr_node_parent->type = z2ui5_if_ajson_types=>node_type-array.
            ls_new_node-index = lcl_utils=>validate_array_index( iv_path  = lv_cur_path
  ...
  METHOD z2ui5_if_ajson~filter.
    ri_json = create_from( ii_source_json = me
                           ii_filter      = ii_filter ).
  ENDMETHOD.
  ...
  METHOD z2ui5_if_ajson~push.
    ...
    lt_new_nodes = lcl_abap_to_json=>convert( is_opts   = ms_opts
                                              iv_data   = iv_val
  ...
  METHOD z2ui5_if_ajson~set.
    ...
    IF ls_split_path IS INITIAL. " Assign root, exceptional processing
      IF iv_node_type IS NOT INITIAL.
        mt_json_tree = lcl_abap_to_json=>insert_with_type( is_opts           = ms_opts
                                                           iv_data           = iv_val
                                                           iv_type           = iv_node_type
                                                           is_prefix         = ls_split_path
                                                           ii_custom_mapping = mi_custom_mapping ).
      ELSE.
        mt_json_tree = lcl_abap_to_json=>convert( is_opts           = ms_opts
                                                  iv_data           = iv_val
                                                  is_prefix         = ls_split_path
                                                  ii_custom_mapping = mi_custom_mapping ).
      ENDIF.
    ...
    ls_deleted_node = delete_subtree( ir_parent = lr_parent
    ...
    IF lr_parent->type = z2ui5_if_ajson_types=>node_type-array.
      lv_array_index = lcl_utils=>validate_array_index( iv_path  = ls_split_path-path
    ...
    IF iv_node_type IS NOT INITIAL.
      lt_new_nodes = lcl_abap_to_json=>insert_with_type( is_opts           = ms_opts
  ...
  METHOD z2ui5_if_ajson~setx.
    ...
    ELSEIF lv_val = 'true'.
      z2ui5_if_ajson~set_boolean( iv_path = lv_path
                                  iv_val  = abap_true ).
    ELSEIF lv_val = 'false'.
      z2ui5_if_ajson~set_boolean( iv_path = lv_path
                                  iv_val  = abap_false ).
    ...
    ELSE. " string
      ...
      IF lv_val+0(1) = '"' AND lv_val+lv_last(1) = '"'.
        lv_val = substring( val = lv_val
                            ...
                            len = lv_last - 1 ).
      ENDIF.
      z2ui5_if_ajson~set_string( iv_path = lv_path
                                 iv_val  = lv_val ).
    ENDIF.
  ...
  METHOD z2ui5_if_ajson~set_boolean.
    ...
    z2ui5_if_ajson~set( iv_ignore_empty = abap_false
                        iv_path         = iv_path
                        iv_val          = lv_bool ).
  ...
  METHOD z2ui5_if_ajson~slice.
    ...
    READ TABLE mt_json_tree INTO ls_item
         WITH KEY path = ls_path_parts-path
                  name = ls_path_parts-name.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    CLEAR: ls_item-path,
           ls_item-name,
           ls_item-order. " this becomes a new root
  ...
  METHOD z2ui5_if_ajson~touch_array.
    ...
    IF iv_clear = abap_true.
      ls_deleted_node = delete_subtree( iv_path = ls_split_path-path
                                        iv_name = ls_split_path-name ).
    ELSE.
      lr_node = get_item( iv_path ).
    ENDIF.
  ...
  METHOD z2ui5_if_ajson~to_abap.
    ...
    lo_to_abap->to_abap( EXPORTING it_nodes    = z2ui5_if_ajson~mt_json_tree
                         CHANGING  c_container = ev_container ).
