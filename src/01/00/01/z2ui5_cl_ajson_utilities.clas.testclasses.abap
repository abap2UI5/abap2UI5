...
CLASS lcl_nodes_helper IMPLEMENTATION.
  METHOD add.
    ...
    SPLIT iv_str AT '|' INTO
      ...
      lv_index
      lv_children.
    <n>-path     = condense( <n>-path ).
    <n>-name     = condense( <n>-name ).
    <n>-type     = condense( <n>-type ).
    <n>-value    = condense( <n>-value ).
    <n>-index    = lv_index.
...
CLASS ltcl_json_utils DEFINITION FINAL
  FOR TESTING RISK LEVEL HARMLESS DURATION SHORT.
...
CLASS ltcl_json_utils IMPLEMENTATION.
  METHOD json_diff.
    ...
    DATA lo_util       TYPE REF TO z2ui5_cl_ajson_utilities.
    DATA lo_insert     TYPE REF TO z2ui5_if_ajson.
    DATA lo_delete     TYPE REF TO z2ui5_if_ajson.
    DATA lo_change     TYPE REF TO z2ui5_if_ajson.
    DATA lo_insert_exp TYPE REF TO lcl_nodes_helper.
    DATA lo_delete_exp TYPE REF TO lcl_nodes_helper.
    DATA lo_change_exp TYPE REF TO lcl_nodes_helper.
...
*      '        "row": 4,\n' && " array delete
    ...
    lo_util->diff( EXPORTING iv_json_a = ltcl_parser_test=>sample_json( )
                             iv_json_b = lv_json
                   IMPORTING eo_insert = lo_insert
                             eo_delete = lo_delete
                             eo_change = lo_change ).

    cl_abap_unit_assert=>assert_equals( exp = lo_insert_exp->mt_nodes
                                        act = lo_insert->mt_json_tree ).

    cl_abap_unit_assert=>assert_equals( exp = lo_delete_exp->mt_nodes
                                        act = lo_delete->mt_json_tree ).

    cl_abap_unit_assert=>assert_equals( exp = lo_change_exp->mt_nodes
  ...
  METHOD json_diff_types.
    ...
    DATA lv_json_a     TYPE string.
    DATA lv_json_b     TYPE string.
    DATA lo_util       TYPE REF TO z2ui5_cl_ajson_utilities.
    DATA lo_insert     TYPE REF TO z2ui5_if_ajson.
    DATA lo_delete     TYPE REF TO z2ui5_if_ajson.
    DATA lo_change     TYPE REF TO z2ui5_if_ajson.
    ...
    lo_util->diff( EXPORTING iv_json_a = lv_json_a
                             iv_json_b = lv_json_b
                   IMPORTING eo_insert = lo_insert
                             eo_delete = lo_delete
                             eo_change = lo_change ).
    ...
    lo_util->diff( EXPORTING iv_json_a = lv_json_b
                             iv_json_b = lv_json_a
                   IMPORTING eo_insert = lo_insert
                             eo_delete = lo_delete
                             eo_change = lo_change ).
  ...
  METHOD json_merge.
    ...
    DATA lv_json_b    TYPE string.
    DATA lo_util      TYPE REF TO z2ui5_cl_ajson_utilities.
    DATA lo_merge     TYPE REF TO z2ui5_if_ajson.
    DATA lo_merge_exp TYPE REF TO lcl_nodes_helper.
    ...
    lo_merge = lo_util->merge( iv_json_a = lv_json_a
                               iv_json_b = lv_json_b ).

    cl_abap_unit_assert=>assert_equals( exp = lo_merge_exp->mt_nodes
  ...
  METHOD json_sort.
    ...
    DATA lv_sorted_exp TYPE string.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lv_sorted_exp
                                        act = lv_sorted ).
  ...
  METHOD is_equal.
    ...
    cl_abap_unit_assert=>assert_true( z2ui5_cl_ajson_utilities=>new( )->is_equal(
                                          ii_json_a = z2ui5_cl_ajson=>parse( '{"a":1,"b":2}' )
                                          ii_json_b = z2ui5_cl_ajson=>parse( '{"a":1,"b":2}' ) ) ).

    cl_abap_unit_assert=>assert_true( z2ui5_cl_ajson_utilities=>new( )->is_equal( iv_json_a = '{"a":1,"b":2}'
                                                                                  iv_json_b = '{"a":1,"b":2}' ) ).

    cl_abap_unit_assert=>assert_false( z2ui5_cl_ajson_utilities=>new( )->is_equal( iv_json_a = '{"a":1,"b":2}'
                                                                                   iv_json_b = '{"a":1,"b":3}' ) ).

    cl_abap_unit_assert=>assert_false( z2ui5_cl_ajson_utilities=>new( )->is_equal(
                                           iv_json_a = '{"a":1,"b":2}'
                                           iv_json_b = '{"a":1,"b":2,"c":3}' ) ).

    cl_abap_unit_assert=>assert_false( z2ui5_cl_ajson_utilities=>new( )->is_equal( iv_json_a = '{"a":1,"b":2,"c":3}'
                                                                                   iv_json_b = '{"a":1,"b":2}' ) ).
