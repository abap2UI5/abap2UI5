...
DATA lr_filter LIKE REF TO <temp197>.
    ...
    FIELD-SYMBOLS <temp31> LIKE LINE OF result.
...
GET REFERENCE OF <temp197> INTO lr_filter.
    ...
    temp32 = sy-tabix.
    READ TABLE result WITH KEY name = name ASSIGNING <temp31>.
    sy-tabix = temp32.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD filter_get_token_t_by_range_t.
    ...
    DATA lr_row LIKE REF TO temp202.
      ...
      FIELD-SYMBOLS <temp33> LIKE LINE OF lt_mapping.
    ...
    LOOP AT lt_tab REFERENCE INTO lr_row.
      ...
      temp34 = sy-tabix.
      READ TABLE lt_mapping WITH KEY n = lr_row->option ASSIGNING <temp33>.
      sy-tabix = temp34.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
  ...
  METHOD itab_filter_by_val.
    ...
    DATA lv_search LIKE temp204.
      ...
      DATA lv_index TYPE i.
          ...
          FIELD-SYMBOLS <temp35> LIKE LINE OF fields.
    ...
    LOOP AT tab ASSIGNING <row>.
      ...
      DO.
        ...
        ELSE.
          ...
          temp36 = sy-tabix.
          READ TABLE fields INDEX lv_index ASSIGNING <temp35>.
          sy-tabix = temp36.
          IF sy-subrc <> 0.
            ASSERT 1 = 0.
          ENDIF.
...
DATA lt_cols TYPE temp2.
    FIELD-SYMBOLS <temp38> LIKE LINE OF lt_rows.
    ...
    temp39 = sy-tabix.
    READ TABLE lt_rows INDEX 1 ASSIGNING <temp38>.
    sy-tabix = temp39.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
...
DATA lt_param TYPE temp3.
  ...
  METHOD msg_get.
    ...
    FIELD-SYMBOLS <temp250> LIKE LINE OF lt_msg.
    DATA temp251 LIKE sy-tabix.
    lt_msg = msg_get_t( val = val val2 = val2 ).


    temp251 = sy-tabix.
    READ TABLE lt_msg INDEX 1 ASSIGNING <temp250>.
    sy-tabix = temp251.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD ui5_msg_box_format.
    ...
    DATA lt_msg TYPE z2ui5_cl_util=>ty_t_msg.
      FIELD-SYMBOLS <temp271> LIKE LINE OF lt_msg.
      DATA temp272 LIKE sy-tabix.
      FIELD-SYMBOLS <temp273> LIKE LINE OF lt_msg.
      DATA temp274 LIKE sy-tabix.
      FIELD-SYMBOLS <temp275> LIKE LINE OF lt_msg.
      ...
      FIELD-SYMBOLS <temp279> LIKE LINE OF lt_msg.
      DATA temp280 LIKE sy-tabix.
      FIELD-SYMBOLS <temp281> LIKE LINE OF lt_msg.
    ...
    IF lines( lt_msg ) = 1.
      ...
      temp272 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp271>.
      sy-tabix = temp272.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      ...
      temp274 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp273>.
      sy-tabix = temp274.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      ...
      temp276 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp275>.
      sy-tabix = temp276.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
    ...
    ELSEIF lines( lt_msg ) > 1.
      ...
      temp280 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp279>.
      sy-tabix = temp280.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      ...
      temp282 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp281>.
      sy-tabix = temp282.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
...
DATA lt_impl TYPE temp7.
  ...
  METHOD msg_get_text.
    ...
    DATA lt_msg TYPE z2ui5_cl_util=>ty_t_msg.
      FIELD-SYMBOLS <temp315> LIKE LINE OF lt_msg.
    ...
    IF lt_msg IS NOT INITIAL.
      ...
      temp316 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp315>.
      sy-tabix = temp316.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
