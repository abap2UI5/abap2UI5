...
CLASS ltcl_unit_test_msg_mapper IMPLEMENTATION.
  ...
  METHOD test_sy.
    ...
    FIELD-SYMBOLS <temp1> LIKE LINE OF lt_result.
    ...
    temp2 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp1>.
    sy-tabix = temp2.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp6 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp5>.
    sy-tabix = temp6.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_bapiret.
    ...
    DATA lt_result TYPE z2ui5_cl_util=>ty_t_msg.
    FIELD-SYMBOLS <temp1> LIKE LINE OF lt_msg.
    DATA temp2 LIKE sy-tabix.
    ...
    temp2 = sy-tabix.
    READ TABLE lt_msg INDEX 1 ASSIGNING <temp1>.
    sy-tabix = temp2.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp14 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp13>.
    sy-tabix = temp14.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_bapirettab.
    ...
    temp20 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp19>.
    sy-tabix = temp20.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp22 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp21>.
    sy-tabix = temp22.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_cx.
        ...
        DATA lt_result TYPE z2ui5_cl_util=>ty_t_msg.
    FIELD-SYMBOLS <temp23> LIKE LINE OF lt_result.
    ...
    temp24 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp23>.
    sy-tabix = temp24.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_rap_via_get.
    ...
    DATA lt_result TYPE z2ui5_cl_util=>ty_t_msg.
    FIELD-SYMBOLS <temp31> LIKE LINE OF lt_result.
    ...
    temp32 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp31>.
    sy-tabix = temp32.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_fallback.
    ...
    DATA lt_result TYPE z2ui5_cl_util=>ty_t_msg.
    FIELD-SYMBOLS <temp40> LIKE LINE OF lt_result.
    ...
    temp41 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp40>.
    sy-tabix = temp41.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_fallback_skip.
    ...
    DATA lt_result TYPE z2ui5_cl_util=>ty_t_msg.
    FIELD-SYMBOLS <temp46> LIKE LINE OF lt_result.
    DATA temp47 LIKE sy-tabix.
    FIELD-SYMBOLS <temp48> LIKE LINE OF lt_result.
    ...
    temp47 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp46>.
    sy-tabix = temp47.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp49 = sy-tabix.
    READ TABLE lt_result INDEX 1 ASSIGNING <temp48>.
    sy-tabix = temp49.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_rap_element.
    ...
    FIELD-SYMBOLS <temp3> LIKE LINE OF lt_result.
    DATA temp4 LIKE sy-tabix.
    FIELD-SYMBOLS <temp60> LIKE LINE OF lt_meta.
    DATA temp61 LIKE sy-tabix.
    FIELD-SYMBOLS <temp62> LIKE LINE OF lt_meta.
    DATA temp63 LIKE sy-tabix.
    FIELD-SYMBOLS <temp64> LIKE LINE OF lt_meta.
    DATA temp65 LIKE sy-tabix.
    FIELD-SYMBOLS <temp66> LIKE LINE OF lt_meta.
    DATA temp67 LIKE sy-tabix.
    FIELD-SYMBOLS <temp68> LIKE LINE OF lt_meta.
    DATA temp69 LIKE sy-tabix.
    FIELD-SYMBOLS <temp70> LIKE LINE OF lt_meta.
    DATA temp71 LIKE sy-tabix.
    FIELD-SYMBOLS <temp72> LIKE LINE OF lt_meta.
    ...
    temp61 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `element` ASSIGNING <temp60>.
    sy-tabix = temp61.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp63 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `state_area` ASSIGNING <temp62>.
    sy-tabix = temp63.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp65 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `action` ASSIGNING <temp64>.
    sy-tabix = temp65.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp67 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `pid` ASSIGNING <temp66>.
    sy-tabix = temp67.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp69 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `cid` ASSIGNING <temp68>.
    sy-tabix = temp69.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp71 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `tky` ASSIGNING <temp70>.
    sy-tabix = temp71.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp73 = sy-tabix.
    READ TABLE lt_meta WITH KEY n = `tky` ASSIGNING <temp72>.
    sy-tabix = temp73.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
