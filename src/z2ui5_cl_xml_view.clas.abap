CLASS z2ui5_cl_xml_view DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS harvey_ball_micro_chart_item
      IMPORTING
        !color         TYPE clike OPTIONAL
    ...
    METHODS viz_data_dimensions
      RETURNING
        VALUE(result) TYPE REF TO z2ui5_cl_xml_view.

    METHODS viz_data_dimension_definition
      IMPORTING !name         TYPE clike OPTIONAL
                !value        TYPE clike OPTIONAL
      RETURNING
                VALUE(result) TYPE REF TO z2ui5_cl_xml_view.

    METHODS viz_data_measures
      RETURNING
        VALUE(result) TYPE REF TO z2ui5_cl_xml_view.

    METHODS viz_data_measure_definition
      IMPORTING !name         TYPE clike OPTIONAL
                !value        TYPE clike OPTIONAL
      RETURNING
                VALUE(result) TYPE REF TO z2ui5_cl_xml_view.
...
*                       ( n = `xmlns:tnt `      v = `sap.tnt` ) ).
  ...
  METHOD harvey_ball_micro_chart_item.
    result = me.
    _generic( name   = `HarveyBallMicroChart`
              ns     = `mchart`
              t_prop = VALUE #( ( n = `color`  v = color )
  ...
  METHOD horizontal_layout.
    result = _generic( name   = `HorizontalLayout`
                       ...
                       t_prop = VALUE #( ( n = `class`  v = class )
                                         ( n = `width`  v = width )
                                         ( n = `allowWrapping`  v = allowwrapping ) ) ).
  ENDMETHOD.
  ...
  METHOD image.
    ...
    _generic( name   = `Image`
              t_prop = VALUE #(
                ...
                ( n = `load` v = load )
                ( n = `decorative` v =  z2ui5_cl_fw_utility=>boolean_abap_2_json( decorative ) )
                ( n = `densityAware` v =  z2ui5_cl_fw_utility=>boolean_abap_2_json( densityaware ) )
  ...
  METHOD list.
    result = _generic( name   = `List`
                       t_prop = VALUE #( ( n = `headerText`             v = headertext )
                                         ...
                                         ( n = `showUnread`             v = z2ui5_cl_fw_utility=>boolean_abap_2_json( showunread ) )
                                         ( n = `visible`                v = z2ui5_cl_fw_utility=>boolean_abap_2_json( visible ) )
                                         ( n = `noData`                 v = nodata ) ) ).
  ENDMETHOD.
  ...
  METHOD object_header.
    ...
    result = _generic( name   = `ObjectHeader`
                       t_prop = VALUE #( ( n = `backgrounddesign`     v = backgrounddesign )
                                         ( n = `condensed`            v = z2ui5_cl_fw_utility=>boolean_abap_2_json( condensed ) )
                                         ( n = `fullscreenoptimized`  v = z2ui5_cl_fw_utility=>boolean_abap_2_json( fullscreenoptimized ) )
                                         ( n = `icon`                 v = icon )
                                         ( n = `iconactive`           v = z2ui5_cl_fw_utility=>boolean_abap_2_json( iconactive ) )
                                         ( n = `iconalt`              v = iconalt )
                                         ( n = `icondensityaware`     v = z2ui5_cl_fw_utility=>boolean_abap_2_json( icondensityaware ) )
                                         ...
                                         ( n = `intro`                v = intro )
                                         ( n = `introactive`          v = z2ui5_cl_fw_utility=>boolean_abap_2_json( introactive ) )
                                         ...
                                         ( n = `responsive`           v = z2ui5_cl_fw_utility=>boolean_abap_2_json( responsive ) )
                                         ( n = `showtitleselector`    v = z2ui5_cl_fw_utility=>boolean_abap_2_json( showtitleselector ) )
                                         ( n = `title`                v = title )
                                         ( n = `titleactive`          v = z2ui5_cl_fw_utility=>boolean_abap_2_json( titleactive ) )
  ...
  METHOD viz_data_dimensions.
    result = _generic( name   = `dimensions`
                       ns     = `viz.data` ).
  ENDMETHOD.


  METHOD viz_data_dimension_definition.
    result = me.
    _generic( name   = `DimensionDefinition`
                       ns     = `viz.data`
                       t_prop = VALUE #(
                         ( n = `name`  v = name )
                         ( n = `value` v = value ) ) ).
  ...
  METHOD viz_data_measures.
    result = _generic( name   = `measures`
                       ns     = `viz.data` ).
  ENDMETHOD.


  METHOD viz_data_measure_definition.
    result = me.
    _generic( name   = `MeasureDefinition`
                       ns     = `viz.data`
                       t_prop = VALUE #(
                         ( n = `name`  v = name )
                         ( n = `value` v = value ) ) ).
