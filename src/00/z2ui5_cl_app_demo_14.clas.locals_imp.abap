...
` csp, cssound_score, cssound_orchestra, cssound_document`.
    DATA lt_types TYPE STANDARD TABLE OF string WITH DEFAULT KEY.
    SPLIT lv_types AT ',' INTO TABLE lt_types.


    DATA temp2 TYPE ty_t_suggest.
    CLEAR temp2.
    DATA row LIKE LINE OF lt_types.
    LOOP AT lt_types INTO row.
      DATA temp3 LIKE LINE OF temp2.
      temp3-name = shift_right( shift_left( row ) ).
      temp3-value = shift_right( shift_left( row ) ).
