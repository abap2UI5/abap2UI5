...
DATA lt_data TYPE temp4.
    ...
    DATA temp186 TYPE ty_row.
    DATA temp187 TYPE ty_row.
    DATA temp188 TYPE ty_row.
    DATA temp189 LIKE LINE OF lt_data.
    DATA temp190 LIKE sy-tabix.
    DATA temp191 LIKE lt_data.
    DATA temp192 LIKE LINE OF temp191.
    DATA temp193 LIKE LINE OF lt_data.
    DATA temp194 LIKE sy-tabix.
    CLEAR temp186.
    temp186-name = `alpha`.
    temp186-value = `low`.
    INSERT temp186 INTO TABLE lt_data.

    CLEAR temp187.
    temp187-name = `beta`.
    temp187-value = `mid`.
    INSERT temp187 INTO TABLE lt_data.

    CLEAR temp188.
    temp188-name = `gamma`.
    temp188-value = `top`.
    INSERT temp188 INTO TABLE lt_data.
    ...
    temp190 = sy-tabix.
    READ TABLE lt_data INDEX 1 INTO temp189.
    sy-tabix = temp190.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `beta` act = temp189-name ).
    ...
    temp194 = sy-tabix.
    READ TABLE lt_data INDEX 1 INTO temp193.
    sy-tabix = temp194.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `beta` act = temp193-name ).
...
DATA lt_data TYPE temp5.

    DATA temp195 TYPE ty_row.
    DATA temp196 TYPE ty_row.
    DATA temp197 TYPE ty_row.
    DATA temp198 TYPE string_table.
    DATA temp200 LIKE LINE OF lt_data.
    DATA temp201 LIKE sy-tabix.
    CLEAR temp195.
    temp195-name = `alpha`.
    temp195-value = `low`.
    INSERT temp195 INTO TABLE lt_data.

    CLEAR temp196.
    temp196-name = `beta`.
    temp196-value = `mid`.
    INSERT temp196 INTO TABLE lt_data.

    CLEAR temp197.
    temp197-name = `low`.
    temp197-value = `top`.
    INSERT temp197 INTO TABLE lt_data.


    CLEAR temp198.
    INSERT `NAME` INTO TABLE temp198.
    ...
    temp201 = sy-tabix.
    READ TABLE lt_data INDEX 1 INTO temp200.
    sy-tabix = temp201.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `top` act = temp200-value ).
  ...
  METHOD test_update_tokens.
    ...
    DATA temp93 LIKE LINE OF temp206-t_token.
    ...
    DATA temp212 LIKE LINE OF lt_result.
    DATA temp213 LIKE sy-tabix.
    DATA temp214 LIKE LINE OF lt_result.
    DATA temp215 LIKE sy-tabix.
    DATA temp95 LIKE LINE OF temp214-t_range.
    DATA temp96 LIKE sy-tabix.
    DATA temp216 LIKE LINE OF lt_result.
    DATA temp217 LIKE sy-tabix.
    DATA temp97 LIKE LINE OF temp216-t_range.
    ...
    temp94 = sy-tabix.
    READ TABLE temp206-t_token INDEX 1 INTO temp93.
    sy-tabix = temp94.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_initial( temp208-t_token_added ).
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_initial( temp210-t_token_removed ).


    temp213 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp212.
    sy-tabix = temp213.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 1 act = lines( temp212-t_range ) ).


    temp215 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp214.
    sy-tabix = temp215.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp96 = sy-tabix.
    READ TABLE temp214-t_range INDEX 1 INTO temp95.
    sy-tabix = temp96.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp217 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp216.
    sy-tabix = temp217.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp98 = sy-tabix.
    READ TABLE temp216-t_range INDEX 1 INTO temp97.
    sy-tabix = temp98.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_update_tokens_remove.
    ...
    DATA temp218 TYPE z2ui5_cl_util=>ty_t_filter_multi.
    DATA temp219 LIKE LINE OF temp218.
    DATA temp99 TYPE z2ui5_cl_util=>ty_t_token.
    DATA temp100 LIKE LINE OF temp99.
    DATA temp101 TYPE z2ui5_cl_util=>ty_t_token.
    DATA temp102 LIKE LINE OF temp101.
    DATA lt_filter LIKE temp218.
    DATA lt_result TYPE z2ui5_cl_util=>ty_t_filter_multi.
    DATA temp220 LIKE LINE OF lt_result.
    DATA temp221 LIKE sy-tabix.
    DATA temp222 LIKE LINE OF lt_result.
    DATA temp223 LIKE sy-tabix.
    DATA temp103 LIKE LINE OF temp222-t_token.
    DATA temp104 LIKE sy-tabix.
    DATA temp224 LIKE LINE OF lt_result.
    DATA temp225 LIKE sy-tabix.
    DATA temp226 LIKE LINE OF lt_result.
    DATA temp227 LIKE sy-tabix.
    DATA temp105 LIKE LINE OF temp226-t_range.
    ...
    temp102-text = `=A`.
    INSERT temp102 INTO TABLE temp101.
    temp219-t_token_removed = temp101.
    ...
    temp221 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp220.
    sy-tabix = temp221.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 1 act = lines( temp220-t_token ) ).


    temp223 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp222.
    sy-tabix = temp223.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp104 = sy-tabix.
    READ TABLE temp222-t_token INDEX 1 INTO temp103.
    sy-tabix = temp104.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp225 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp224.
    sy-tabix = temp225.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 1 act = lines( temp224-t_range ) ).


    temp227 = sy-tabix.
    READ TABLE lt_result INDEX 1 INTO temp226.
    sy-tabix = temp227.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    ...
    temp106 = sy-tabix.
    READ TABLE temp226-t_range INDEX 1 INTO temp105.
    sy-tabix = temp106.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
...
CLASS ltcl_unit_test_conversion IMPLEMENTATION.
  ...
  METHOD test_json_roundtrip.
    ...
    DATA temp228 TYPE ty_data.
    DATA ls_in LIKE temp228.
    DATA lv_json TYPE string.
    DATA ls_out TYPE ty_data.
    CLEAR temp228.
    temp228-name = `test`.
    temp228-value = 42.
    temp228-flag = abap_true.
...
DATA lt_in TYPE temp6.
    DATA temp229 LIKE lt_in.
  ...
  METHOD test_conv_date_to_string.
    ...
    lv_str = z2ui5_cl_util=>conv_date_to_string( temp231 ).
  ...
  METHOD test_itab_get_by_struc.
    ...
    DATA temp232 TYPE ty_s.
    DATA ls_struc LIKE temp232.
    DATA lt_nv TYPE z2ui5_cl_util=>ty_t_name_value.
    DATA temp233 LIKE LINE OF lt_nv.
    DATA temp234 LIKE sy-tabix.
    DATA temp235 LIKE LINE OF lt_nv.
    DATA temp236 LIKE sy-tabix.
    DATA temp237 LIKE LINE OF lt_nv.
    DATA temp238 LIKE sy-tabix.
    DATA temp239 LIKE LINE OF lt_nv.
    DATA temp240 LIKE sy-tabix.
    CLEAR temp232.
    temp232-name = `test_name`.
    temp232-value = `test_value`.
    ...
    temp234 = sy-tabix.
    READ TABLE lt_nv INDEX 1 INTO temp233.
    sy-tabix = temp234.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `NAME`       act = temp233-n ).


    temp236 = sy-tabix.
    READ TABLE lt_nv INDEX 1 INTO temp235.
    sy-tabix = temp236.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `test_name`  act = temp235-v ).


    temp238 = sy-tabix.
    READ TABLE lt_nv INDEX 2 INTO temp237.
    sy-tabix = temp238.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `VALUE`      act = temp237-n ).


    temp240 = sy-tabix.
    READ TABLE lt_nv INDEX 2 INTO temp239.
    sy-tabix = temp240.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `test_value` act = temp239-v ).
  ...
  METHOD test_cal_weekday.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = 1
        act = z2ui5_cl_util=>cal_get_weekday( temp241 ) ).

    temp242 = `20240101`.
    cl_abap_unit_assert=>assert_false(
        z2ui5_cl_util=>cal_is_weekend( temp242 ) ).


    temp243 = `20240107`.
    cl_abap_unit_assert=>assert_equals(
        exp = 7
        act = z2ui5_cl_util=>cal_get_weekday( temp243 ) ).

    temp244 = `20240107`.
    cl_abap_unit_assert=>assert_true(
        z2ui5_cl_util=>cal_is_weekend( temp244 ) ).
  ...
  METHOD test_cal_workdays.
    ...
    cl_abap_unit_assert=>assert_equals(
        exp = 5
        act = z2ui5_cl_util=>cal_count_workdays( date_from = temp246
                                                 date_to   = temp108 ) ).
  ...
  METHOD test_zip_pack.
    ...
    temp248-name = `hello.txt`.

    temp109 = `48656C6C6F`.
    temp248-content = temp109.
    INSERT temp248 INTO TABLE temp247.
    temp248-name = `world.txt`.

    temp110 = `576F726C64`.
    temp248-content = temp110.
    INSERT temp248 INTO TABLE temp247.
  ...
  METHOD test_zip_unpack.
    ...
    DATA temp249 TYPE z2ui5_cl_util=>ty_t_zip_file.
    DATA temp250 LIKE LINE OF temp249.
    DATA temp111 TYPE xstring.
    DATA temp112 TYPE xstring.
    DATA lt_in LIKE temp249.
    DATA lv_archive TYPE xstring.
    DATA lt_out TYPE z2ui5_cl_util=>ty_t_zip_file.
    DATA ls_in LIKE LINE OF lt_in.
      DATA temp251 TYPE z2ui5_cl_util=>ty_s_zip_file.
      DATA temp252 TYPE z2ui5_cl_util=>ty_s_zip_file.
      DATA ls_out LIKE temp251.
    CLEAR temp249.

    temp250-name = `hello.txt`.

    temp111 = `48656C6C6F`.
    temp250-content = temp111.
    INSERT temp250 INTO TABLE temp249.
    temp250-name = `world.txt`.

    temp112 = `576F726C64`.
    temp250-content = temp112.
    INSERT temp250 INTO TABLE temp249.
    ...
    LOOP AT lt_in INTO ls_in.
      ...
      READ TABLE lt_out INTO temp252 WITH KEY name = ls_in-name.
      IF sy-subrc = 0.
        temp251 = temp252.
      ENDIF.
