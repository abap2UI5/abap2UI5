...
CLASS ltcl_unit_test_conversion IMPLEMENTATION.
  ...
  METHOD test_json_roundtrip.
    ...
    DATA temp77 TYPE ty_data.
    DATA ls_in LIKE temp77.
    DATA lv_json TYPE string.
    DATA ls_out TYPE ty_data.
    CLEAR temp77.
    temp77-name = `test`.
    temp77-value = 42.
    temp77-flag = abap_true.
...
DATA lt_in TYPE temp3.
  ...
  METHOD test_conv_date_to_string.
    ...
    lv_str = z2ui5_cl_util=>conv_date_to_string( temp80 ).
  ...
  METHOD test_itab_get_by_struc.
    ...
    DATA temp81 TYPE ty_s.
    DATA ls_struc LIKE temp81.
    DATA lt_nv TYPE z2ui5_cl_util=>ty_t_name_value.
    DATA temp82 LIKE LINE OF lt_nv.
    DATA temp83 LIKE sy-tabix.
    DATA temp84 LIKE LINE OF lt_nv.
    DATA temp85 LIKE sy-tabix.
    DATA temp86 LIKE LINE OF lt_nv.
    DATA temp87 LIKE sy-tabix.
    DATA temp88 LIKE LINE OF lt_nv.
    DATA temp89 LIKE sy-tabix.
    CLEAR temp81.
    temp81-name = `test_name`.
    temp81-value = `test_value`.
    ...
    temp83 = sy-tabix.
    READ TABLE lt_nv INDEX 1 INTO temp82.
    sy-tabix = temp83.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `NAME`       act = temp82-n ).
    
    
    temp85 = sy-tabix.
    READ TABLE lt_nv INDEX 1 INTO temp84.
    sy-tabix = temp85.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `test_name`  act = temp84-v ).
    
    
    temp87 = sy-tabix.
    READ TABLE lt_nv INDEX 2 INTO temp86.
    sy-tabix = temp87.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `VALUE`      act = temp86-n ).
    
    
    temp89 = sy-tabix.
    READ TABLE lt_nv INDEX 2 INTO temp88.
    sy-tabix = temp89.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = `test_value` act = temp88-v ).
