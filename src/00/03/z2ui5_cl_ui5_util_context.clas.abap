...
CLASS z2ui5_cl_ui5_util_context IMPLEMENTATION.
  ...
  METHOD boolean_check_by_data.
        ...
        DATA temp77 TYPE z2ui5_cl_ui5_util_context=>ty_s_bool_cache.
    ...
    TRY.
        ...
        CLEAR temp77.
        temp77-typedescr = lo_descr.
        temp77-is_bool = result.
        INSERT temp77 INTO TABLE mt_bool_cache.
  ...
  METHOD filter_get_token_range_mapping.
    ...
    DATA temp78 TYPE z2ui5_cl_ui5_util_context=>ty_t_name_value.
  ...
  METHOD filter_get_token_t_by_range_t.
    ...
    DATA temp80 LIKE LINE OF lt_tab.
    DATA lr_row LIKE REF TO temp80.
      DATA lv_value TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value-v.
      FIELD-SYMBOLS <temp4> LIKE LINE OF lt_mapping.
      DATA temp5 LIKE sy-tabix.
      DATA temp81 TYPE z2ui5_cl_ui5_util_context=>ty_s_token.
    ...
    LOOP AT lt_tab REFERENCE INTO lr_row.
      ...
      CLEAR temp81.
      temp81-key = lv_value.
      temp81-text = lv_value.
      temp81-visible = abap_true.
      temp81-editable = abap_true.
      INSERT temp81 INTO TABLE result.
    ENDLOOP.
  ...
  METHOD rtti_check_class_exists.
    ...
    DATA temp83 TYPE z2ui5_cl_ui5_util_context=>ty_s_class_exists.
    ...
    CLEAR temp83.
    temp83-name = lv_name.
    temp83-exists = result.
    INSERT temp83 INTO TABLE gt_class_exists.

  ENDMETHOD.

  METHOD rtti_check_ref_data.
        DATA lo_typdescr TYPE REF TO cl_abap_typedescr.
        DATA temp84 TYPE REF TO cl_abap_refdescr.
    ...
    TRY.
        ...
        lo_referenced = temp84->get_referenced_type( ).
  ...
  METHOD rtti_get_t_attri_by_oref.
    ...
    temp86 ?= cl_abap_objectdescr=>describe_by_object_ref( val ).
...
DATA lt_param TYPE temp1.
  ...
  METHOD rtti_check_table_standard.
    ...
    TRY.
        ...
        temp4 = boolc( temp100->table_kind = cl_abap_tabledescr=>tablekind_std ).
        result = temp4.
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
  ...
  METHOD rtti_create_sel_tab_type.
    ...
    DATA temp101 TYPE REF TO cl_abap_tabledescr.
    DATA lo_table LIKE temp101.
        DATA temp102 TYPE REF TO cl_abap_structdescr.
        DATA lo_struct LIKE temp102.
        DATA temp103 TYPE REF TO cl_abap_elemdescr.
        DATA lo_elem LIKE temp103.
        DATA temp104 TYPE abap_componentdescr.
    DATA temp105 LIKE sy-subrc.
      DATA lo_type_bool TYPE REF TO cl_abap_typedescr.
      DATA temp106 TYPE abap_componentdescr.
      DATA temp9 TYPE REF TO cl_abap_datadescr.
    DATA lo_line_type TYPE REF TO cl_abap_structdescr.
    ASSIGN ir_tab->* TO <tab>.


    temp101 ?= cl_abap_typedescr=>describe_by_data( <tab> ).
    ...
    TRY.
      ...
      CATCH cx_root.
        ...
        temp103 ?= lo_table->get_table_line_type( ).

        lo_elem = temp103.

        CLEAR temp104.
        temp104-name = `TAB_LINE`.
        temp104-type = lo_elem.
        INSERT temp104 INTO TABLE lt_comp.
    ENDTRY.
    ...
    IF add_sel_field = abap_true
        AND NOT temp105 = 0. "#EC CI_SORTSEQ

      lo_type_bool = cl_abap_typedescr=>describe_by_name( `ABAP_BOOL` ).

      CLEAR temp106.
      temp106-name = sel_field_name.

      temp9 ?= lo_type_bool.
      temp106-type = temp9.
      INSERT temp106 INTO TABLE lt_comp.
    ENDIF.
  ...
  METHOD ui5_msg_box_format.
    ...
    DATA temp11 LIKE sy-tabix.
      FIELD-SYMBOLS <temp107> LIKE LINE OF lt_msg.
      DATA temp108 LIKE sy-tabix.
    DATA lt_detail_items TYPE string_table.
    DATA temp109 LIKE LINE OF lt_msg.
    DATA lr_msg LIKE REF TO temp109.
      DATA temp110 LIKE LINE OF lt_detail_items.
    ...
    IF lv_lines = 1.
      ...
      temp108 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp107>.
      sy-tabix = temp108.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-text = <temp107>-text.
      RETURN.
    ENDIF.
    ...
    LOOP AT lt_msg REFERENCE INTO lr_msg.
      ...
      temp110 = |<li>{ c_escape_html( lr_msg->text ) }</li>|.
      INSERT temp110 INTO TABLE lt_detail_items.
    ENDLOOP.
  ...
  METHOD data_render_tab.
    ...
    FIELD-SYMBOLS <row> TYPE ANY.
      ...
      DATA temp112 LIKE LINE OF lt_item.
    ...
    LOOP AT <tab> ASSIGNING <row>.
      ...
      IF lv_no > cv_data_max_rows.
        ...
        temp111 = |<li><em>... { lines( <tab> ) - cv_data_max_rows } more entries</em></li>|.
        INSERT temp111
               INTO TABLE lt_item.
        EXIT.
      ENDIF.

      temp112 = |<li>{ data_render( val = <row> depth = depth + 1 ) }</li>|.
      INSERT temp112 INTO TABLE lt_item.
    ENDLOOP.
  ...
  METHOD data_render_oref.
    ...
    FIELD-SYMBOLS <comp> TYPE any.
        DATA lt_attri TYPE abap_attrdescr_tab.
    DATA temp114 LIKE LINE OF lt_attri.
    DATA lr_attri LIKE REF TO temp114.
  ...
  METHOD data_get_exc_text.
        DATA temp115 TYPE REF TO cx_root.

    TRY.

        temp115 ?= val.
        result = temp115->get_text( ).
      CATCH cx_root ##NO_HANDLER.
  ...
  METHOD rtti_check_serializable.
        DATA temp116 TYPE REF TO if_serializable_object.
        DATA lo_dummy LIKE temp116.
...
DATA lt_impl TYPE temp2.
  ...
  METHOD msg_get_internal.
        FIELD-SYMBOLS <tab> TYPE ANY TABLE.
        DATA lt_attri TYPE abap_component_tab.
        DATA temp117 TYPE ty_s_msg.
        DATA ls_result LIKE temp117.
        DATA temp118 LIKE LINE OF lt_attri.
        DATA ls_attri LIKE REF TO temp118.
          FIELD-SYMBOLS <comp> TYPE any.
          DATA temp119 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    CASE rtti_get_type_kind( val ).
      ...
      WHEN OTHERS.
        ...
        IF rtti_check_clike( val ) IS NOT INITIAL AND val IS NOT INITIAL.
          ...
          CLEAR temp119.
          temp119-text = val.
          INSERT temp119 INTO TABLE result.
        ENDIF.
    ENDCASE.
  ...
  METHOD msg_get_by_oref.
        DATA temp120 TYPE REF TO cx_root.
        DATA lx LIKE temp120.
        DATA temp121 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    DATA obj TYPE REF TO object.
    DATA lr_tab TYPE REF TO data.
    FIELD-SYMBOLS <tab> TYPE any.
    DATA temp122 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    INSERT msg_get_by_oref_attri( io_obj = obj
                                  is_msg = temp122 ) INTO TABLE result.
  ...
  METHOD msg_get_by_oref_attri.
    ...
    DATA temp123 LIKE LINE OF lt_attri_o.
    DATA ls_attri_o LIKE REF TO temp123.
  ...
  METHOD check_is_rap_struct.
    ...
    DATA lt_attri TYPE abap_component_tab.
    DATA temp124 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp124.
          DATA temp125 TYPE REF TO cl_abap_tabledescr.
          DATA lo_line TYPE REF TO cl_abap_datadescr.
          DATA temp126 TYPE REF TO cl_abap_structdescr.
          DATA lt_comps TYPE abap_component_tab.
          DATA temp127 LIKE LINE OF lt_comps.
          DATA ls_comp LIKE REF TO temp127.
    lt_attri = rtti_get_t_attri_by_any( val ).
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      ...
      TRY.
          ...
          temp125 ?= ls_attri->type.

          lo_line = temp125->get_table_line_type( ).
          CHECK lo_line->kind = cl_abap_typedescr=>kind_struct.

          temp126 ?= lo_line.

          lt_comps = temp126->get_components( ).
  ...
  METHOD msg_get_rap.
    ...
    DATA lt_attri TYPE abap_component_tab.
    DATA temp128 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp128.
      FIELD-SYMBOLS <tab> TYPE any.
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      ...
      temp129 ?= ls_attri->type.

      lo_line = temp129->get_table_line_type( ).
  ...
  METHOD msg_get_rap_row.
    ...
    FIELD-SYMBOLS <fail> TYPE any.
      FIELD-SYMBOLS <cause> TYPE any.
        ...
        DATA temp130 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    IF sy-subrc = 0.
      ...
      IF sy-subrc = 0.
        ...
        CLEAR temp130.
        temp130-type = `E`.
        temp130-text = lv_text.
        temp130-t_meta = lt_meta.
        INSERT temp130 INTO TABLE messages.
      ENDIF.
    ENDIF.
  ...
  METHOD scan_flag_prefix.
    ...
    DATA lv_len TYPE i.
    DATA lt_attri TYPE abap_component_tab.
    DATA temp131 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp131.
  ...
  METHOD msg_get_rap_action.
    ...
    DATA temp133 TYPE string.
    lt_suffix = scan_flag_prefix( val    = val
                                        prefix = `%OP-%ACTION-` ).

    CLEAR temp132.

    READ TABLE lt_suffix INTO temp133 INDEX 1.
    IF sy-subrc = 0.
      temp132 = temp133.
    ENDIF.
    result = temp132.
  ...
  METHOD msg_get_rap_flatten.
    ...
    DATA ls_attri LIKE REF TO temp134.
      FIELD-SYMBOLS <comp> TYPE any.
      DATA lv_sub_kind TYPE string.
        DATA lv_sub TYPE string.
            DATA lv_str TYPE string.
            DATA temp135 LIKE LINE OF lt_part.
    lv_kind = rtti_get_type_kind( val ).
    IF lv_kind <> cl_abap_datadescr=>typekind_struct1
        AND lv_kind <> cl_abap_datadescr=>typekind_struct2.
      RETURN.
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      ...
      ELSEIF <comp> IS NOT INITIAL.
        TRY.
            ...
            temp135 = |{ ls_attri->name }={ lv_str }|.
            APPEND temp135 TO lt_part.
          CATCH cx_root ##NO_HANDLER.
        ENDTRY.
      ENDIF.
    ENDLOOP.
  ...
  METHOD msg_get_rap_meta.
    ...
    DATA lv TYPE string.
      ...
      DATA temp139 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp140 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp141 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp141-n = `tky`.
      temp141-v = lv.
      INSERT temp141 INTO TABLE result.
    ENDIF.
  ...
  METHOD msg_get_rap_fail_text.
    ...
    DATA temp142 TYPE string.
    CASE cause.
      WHEN 0.
        temp142 = `Operation failed`.
      WHEN 1.
        temp142 = `Entity not found`.
      WHEN 2.
        temp142 = `Entity is locked`.
      WHEN 3.
        temp142 = `Authorization failure`.
      WHEN 4 OR 5.
        temp142 = `Concurrent modification`.
      WHEN 6.
        temp142 = `Operation disabled`.
      WHEN 7.
        temp142 = `Operation forbidden`.
      WHEN 8.
        temp142 = `Semantic error`.
      WHEN 9.
        temp142 = `Determination failed`.
      WHEN 10.
        temp142 = `Permission denied`.
      WHEN 11.
        temp142 = `Validation failed`.
      WHEN OTHERS.
        temp142 = |Operation failed (cause code { cause })|.
    ENDCASE.
    result = temp142.
