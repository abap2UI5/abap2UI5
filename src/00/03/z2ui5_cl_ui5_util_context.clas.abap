CLASS z2ui5_cl_ui5_util_context DEFINITION
  ...
  PUBLIC SECTION.
    ...
    " GET-shell ETag hashes the UTF-8 bytes of its cache key, so the
    " framework itself calls this again: see z2ui5_cl_ui5_http_handler=>_get_etag
...
CLASS z2ui5_cl_ui5_util_context IMPLEMENTATION.
  ...
  METHOD boolean_check_by_data.
        ...
        DATA temp115 TYPE z2ui5_cl_ui5_util_context=>ty_s_bool_cache.
    ...
    TRY.
        ...
        result = boolean_check_by_name( lo_descr->get_relative_name( ) ).


        CLEAR temp115.
        temp115-typedescr = lo_descr.
        temp115-is_bool = result.
        INSERT temp115 INTO TABLE mt_bool_cache.
  ...
  METHOD filter_get_token_range_mapping.
    ...
    DATA temp116 TYPE z2ui5_cl_ui5_util_context=>ty_t_name_value.
  ...
  METHOD filter_get_token_t_by_range_t.
    ...
    DATA lt_tab TYPE ty_t_range.
    DATA temp118 LIKE LINE OF lt_tab.
    DATA lr_row LIKE REF TO temp118.
      DATA lv_option TYPE string.
      DATA lv_value TYPE string.
      DATA temp119 TYPE string.
      DATA temp120 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
        FIELD-SYMBOLS <temp121> LIKE LINE OF lt_mapping.
        DATA temp122 LIKE sy-tabix.
      DATA temp123 TYPE z2ui5_cl_ui5_util_context=>ty_s_token.
    ...
    LOOP AT lt_tab REFERENCE INTO lr_row.
      ...
      READ TABLE lt_mapping INTO temp120 WITH KEY n = lv_option.
      IF sy-subrc = 0.
        temp119 = temp120-v.
      ENDIF.
      lv_value = temp119. "#EC CI_SORTSEQ
      IF lv_value IS INITIAL.


        temp122 = sy-tabix.
        READ TABLE lt_mapping WITH KEY n = `EQ` ASSIGNING <temp121>.
        sy-tabix = temp122.
        IF sy-subrc <> 0.
          ASSERT 1 = 0.
        ENDIF.
        lv_value = <temp121>-v. "#EC CI_SORTSEQ
      ENDIF.
      ...
      CLEAR temp123.
      temp123-key = lv_value.
      temp123-text = lv_value.
      temp123-visible = abap_true.
      temp123-editable = abap_true.
      INSERT temp123 INTO TABLE result.
    ENDLOOP.
    ...
    LOOP AT tab ASSIGNING <row>.
      ...
      DO.
        ...
        ELSE.
          ...
          temp9 = sy-tabix.
          READ TABLE fields INDEX lv_index ASSIGNING <temp8>.
          sy-tabix = temp9.
          IF sy-subrc <> 0.
            ASSERT 1 = 0.
          ENDIF.
  ...
  METHOD rtti_check_class_exists.
    ...
    DATA temp125 TYPE z2ui5_cl_ui5_util_context=>ty_s_class_exists.
    ...
    CLEAR temp125.
    temp125-name = lv_name.
    temp125-exists = result.
    INSERT temp125 INTO TABLE gt_class_exists.
  ...
  METHOD rtti_check_class_impl_intf.
    ...
    DATA lr_hit TYPE REF TO z2ui5_cl_ui5_util_context=>ty_s_class_impl_intf.
          DATA temp126 LIKE sy-subrc.
          DATA temp3 TYPE xsdboolean.
    DATA temp127 TYPE z2ui5_cl_ui5_util_context=>ty_s_class_impl_intf.
    ...
    READ TABLE gt_class_impl_intf REFERENCE INTO lr_hit
    ...
    TRY.
        ...
        IF sy-subrc = 0.
          ...
          READ TABLE lo_classdescr->interfaces WITH KEY name = lv_intf TRANSPORTING NO FIELDS.
          temp126 = sy-subrc.

          temp3 = boolc( temp126 = 0 ).
          result = temp3. "#EC CI_SORTSEQ
        ENDIF.
    ...
    CLEAR temp127.
    temp127-name = lv_class.
    temp127-intf = lv_intf.
    temp127-result = result.
    INSERT temp127 INTO TABLE gt_class_impl_intf.

  ENDMETHOD.

  METHOD rtti_check_ref_data.
        DATA lo_typdescr TYPE REF TO cl_abap_typedescr.
        DATA temp128 TYPE REF TO cl_abap_refdescr.
    ...
    TRY.
        ...
        lo_referenced = temp128->get_referenced_type( ).

        temp4 = boolc( lo_referenced->kind <> cl_abap_typedescr=>kind_class AND lo_referenced->kind <> cl_abap_typedescr=>kind_intf ).
        result = temp4.
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
  ...
  METHOD rtti_get_classname_by_ref.
    ...
    result = substring_after( val = cl_abap_classdescr=>get_class_name( val )
                              sub = `\CLASS=` ).
      ...
      CATCH cx_sy_move_cast_error INTO lx_cast.
        RAISE EXCEPTION TYPE z2ui5_cx_ui5_util_error
          EXPORTING
            ...
            previous = lx_cast.
    ENDTRY.
    result = expand_components( val   = sdescr->get_components( )
                                depth = depth ).

  ENDMETHOD.

  METHOD expand_components.
    DATA temp129 LIKE LINE OF val.
    DATA lr_comp LIKE REF TO temp129.
  ...
  METHOD rtti_get_t_attri_by_oref.
    ...
    DATA temp130 TYPE REF TO cl_abap_classdescr.
    temp130 ?= cl_abap_objectdescr=>describe_by_object_ref( val ).
    result = temp130->attributes.
  ...
  METHOD rtti_get_t_attri_by_any.
    ...
    DATA lv_absolute_name TYPE string.
        DATA temp131 TYPE REF TO cl_abap_structdescr.
        DATA temp132 TYPE REF TO cl_abap_structdescr.
        DATA temp10 TYPE REF TO cl_abap_tabledescr.
    DATA lr_cache TYPE REF TO z2ui5_cl_ui5_util_context=>ty_s_attri_cache.
      DATA temp133 TYPE z2ui5_cl_ui5_util_context=>ty_s_attri_cache.
    ...
    CASE lo_type->kind.
      ...
      WHEN cl_abap_typedescr=>kind_table.
        ...
        temp10 ?= lo_type.
        temp132 ?= temp10->get_table_line_type( ).
        lo_struct = temp132.
      WHEN OTHERS.
        lo_struct ?= lo_type.
    ENDCASE.
    ...
    result = expand_components( lo_struct->get_components( ) ).
    ...
    ELSE.
      ...
      CLEAR temp133.
      temp133-absolute_name = lv_absolute_name.
      temp133-o_struct = lo_struct.
      temp133-t_attri = result.
      INSERT temp133 INTO TABLE mt_attri_cache.
    ENDIF.
  ...
  METHOD url_param_get.
    ...
    READ TABLE lt_params INTO temp135 WITH KEY n = lv_val.
    IF sy-subrc = 0.
      temp134 = temp135-v.
    ENDIF.
    result = temp134. "#EC CI_SORTSEQ
...
DATA lt_param TYPE temp1.
    DATA temp136 LIKE LINE OF lt_param.
    DATA lr_param LIKE REF TO temp136.
      DATA lv_name TYPE string.
      DATA lv_value TYPE string.
    ...
    IF lv_startup IS NOT INITIAL.
      ...
      lv_before = substring_before( val = |&{ lv_search }|
                                          sub = `&sap-startup-params=` ).
    ...
    LOOP AT lt_param REFERENCE INTO lr_param.
      ...
      CLEAR temp137.
      temp137-n = c_trim_lower( lv_name ).
      temp137-v = lv_value.
  ...
  METHOD xml_srtti_descr.
    ...
    DATA temp138 TYPE string.
    DATA lv_classname LIKE temp138.
    IF rtti_check_class_exists( `ZCL_SRTTI_TYPEDESCR` ) = abap_true.
      temp138 = `ZCL_SRTTI_TYPEDESCR`.
    ELSE.
      temp138 = `Z2UI5_CL_SRT_TYPEDESCR`.
    ENDIF.

    lv_classname = temp138.
  ...
  METHOD itab_get_by_struc.
    ...
    DATA temp139 LIKE LINE OF lt_attri.
    DATA lr_attri LIKE REF TO temp139.
      FIELD-SYMBOLS <component> TYPE any.
          DATA temp140 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
    ...
    LOOP AT lt_attri REFERENCE INTO lr_attri.
      ...
      CASE rtti_get_type_kind( <component> ).
        ...
        WHEN OTHERS.
          ...
          CLEAR temp140.
          temp140-n = lr_attri->name.
          temp140-v = <component>.
          INSERT temp140 INTO TABLE result.
      ENDCASE.
  ...
  METHOD error_get_attributes.
    ...
    DATA temp141 LIKE LINE OF lt_attri.
    DATA lr_attri LIKE REF TO temp141.
      DATA temp142 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
    ...
    LOOP AT lt_attri REFERENCE INTO lr_attri "#EC CI_SORTSEQ
      ...
      CLEAR temp142.
      temp142-n = lv_name.
      temp142-v = c_trim( |{ <comp> }| ).
      INSERT temp142 INTO TABLE result.
    ENDLOOP.
  ...
  METHOD rtti_get_ddic_type_name.
    ...
    DATA temp143 TYPE REF TO cl_abap_elemdescr.
    temp143 ?= type.
    result = substring_after( val = temp143->absolute_name
                              sub = `\TYPE=` ).
  ...
  METHOD rtti_check_table_standard.
        DATA lo_type TYPE REF TO cl_abap_typedescr.
        DATA temp144 TYPE REF TO cl_abap_tabledescr.
    ...
    TRY.
        ...
        temp5 = boolc( temp144->table_kind = cl_abap_tabledescr=>tablekind_std ).
        result = temp5.
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
  ...
  METHOD rtti_create_sel_tab_type.
    ...
    DATA lo_table TYPE REF TO cl_abap_tabledescr.
        DATA temp145 TYPE REF TO cl_abap_tabledescr.
        DATA lx_cast TYPE REF TO cx_root.
    DATA lo_line TYPE REF TO cl_abap_datadescr.
        DATA temp146 TYPE REF TO cl_abap_structdescr.
        DATA lo_struct LIKE temp146.
        DATA temp147 TYPE REF TO cl_abap_elemdescr.
        DATA lo_elem LIKE temp147.
        DATA temp148 TYPE abap_componentdescr.
    DATA temp149 LIKE sy-subrc.
      DATA lo_type_bool TYPE REF TO cl_abap_typedescr.
      DATA temp150 TYPE abap_componentdescr.
    ...
    TRY.
        ...
        temp145 ?= cl_abap_typedescr=>describe_by_data( <tab> ).
        lo_table = temp145.

      CATCH cx_root INTO lx_cast.
        RAISE EXCEPTION TYPE z2ui5_cx_ui5_util_error
          EXPORTING
            val = lx_cast.
    ENDTRY.
    ...
    CASE lo_line->kind.
      ...
      WHEN cv_typedescr_kind_elem.
        ...
        CLEAR temp148.
        temp148-name = `TAB_LINE`.
        temp148-type = lo_elem.
        INSERT temp148 INTO TABLE lt_comp.
      WHEN OTHERS.
        RAISE EXCEPTION TYPE z2ui5_cx_ui5_util_error
          EXPORTING
    ...
    IF add_sel_field = abap_true
        AND NOT temp149 = 0. "#EC CI_SORTSEQ

      lo_type_bool = cl_abap_typedescr=>describe_by_name( `ABAP_BOOL` ).

      CLEAR temp150.
      temp150-name = sel_field_name.

      temp11 ?= lo_type_bool.
      temp150-type = temp11.
      INSERT temp150 INTO TABLE lt_comp.
    ENDIF.
  ...
  METHOD rtti_check_table.
    ...
    temp6 = boolc( rtti_get_type_kind( val ) = cl_abap_typedescr=>typekind_table ).
    result = temp6.
  ...
  METHOD rtti_check_structure.
    ...
    TRY.
        ...
        temp7 = boolc( lo_type->kind = cl_abap_typedescr=>kind_struct ).
        result = temp7.
      CATCH cx_root.
        result = abap_false.
    ENDTRY.
  ...
  METHOD ui5_msg_box_format.
    ...
    FIELD-SYMBOLS <temp12> LIKE LINE OF lt_msg.
    DATA temp13 LIKE sy-tabix.
      FIELD-SYMBOLS <temp151> LIKE LINE OF lt_msg.
      DATA temp152 LIKE sy-tabix.
    DATA lt_detail_items TYPE string_table.
    DATA temp153 LIKE LINE OF lt_msg.
    DATA lr_msg LIKE REF TO temp153.
      DATA temp154 LIKE LINE OF lt_detail_items.
    ...
    temp13 = sy-tabix.
    READ TABLE lt_msg INDEX 1 ASSIGNING <temp12>.
    sy-tabix = temp13.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    lv_type = ui5_get_msg_type( <temp12>-type ).
    ...
    IF lv_lines = 1.
      ...
      temp152 = sy-tabix.
      READ TABLE lt_msg INDEX 1 ASSIGNING <temp151>.
      sy-tabix = temp152.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-text = <temp151>-text.
      RETURN.
    ENDIF.
    ...
    LOOP AT lt_msg REFERENCE INTO lr_msg.
      ...
      temp154 = |<li>{ c_escape_html( lr_msg->text ) }</li>|.
      INSERT temp154 INTO TABLE lt_detail_items.
    ENDLOOP.
    result-details = html_get_list( lt_detail_items ).
  ...
  METHOD data_render_tab.
    ...
    FIELD-SYMBOLS <tab> TYPE ANY TABLE.
    FIELD-SYMBOLS <row> TYPE ANY.
        DATA temp155 LIKE LINE OF lt_item.
      DATA temp156 LIKE LINE OF lt_item.
    ...
    LOOP AT <tab> ASSIGNING <row>.
      ...
      IF lv_no > cv_data_max_rows.
        ...
        temp155 = |<li><em>... { lines( <tab> ) - cv_data_max_rows } more entries</em></li>|.
        INSERT temp155
               INTO TABLE lt_item.
        EXIT.
      ENDIF.

      temp156 = |<li>{ data_render( val = <row> depth = depth + 1 ) }</li>|.
      INSERT temp156 INTO TABLE lt_item.
    ENDLOOP.
  ...
  METHOD data_render_struc.
    ...
    FIELD-SYMBOLS <comp> TYPE any.
        DATA lt_attri TYPE abap_component_tab.
    DATA temp157 LIKE LINE OF lt_attri.
    DATA lr_attri LIKE REF TO temp157.
  ...
  METHOD data_render_oref.
    ...
    FIELD-SYMBOLS <comp> TYPE any.
        DATA lt_attri TYPE abap_attrdescr_tab.
    DATA temp158 LIKE LINE OF lt_attri.
    DATA lr_attri LIKE REF TO temp158.
  ...
  METHOD data_get_exc_text.
        DATA temp159 TYPE REF TO cx_root.

    TRY.

        temp159 ?= val.
        result = temp159->get_text( ).
      CATCH cx_root ##NO_HANDLER.
  ...
  METHOD html_check.
    ...
    temp8 = boolc( val CS `</` OR val CS `<br` OR val CS `<hr` OR val CS `<img` ).
    result = temp8.
  ...
  METHOD rtti_check_serializable.
        DATA temp160 TYPE REF TO if_serializable_object.
        DATA lo_dummy LIKE temp160.
...
DATA lt_impl TYPE temp2.
  ...
  METHOD msg_get_internal.
        FIELD-SYMBOLS <tab> TYPE ANY TABLE.
        DATA lt_attri TYPE abap_component_tab.
        DATA temp161 TYPE ty_s_msg.
        DATA ls_result LIKE temp161.
        DATA temp162 LIKE LINE OF lt_attri.
        DATA ls_attri LIKE REF TO temp162.
          FIELD-SYMBOLS <comp> TYPE any.
          DATA temp163 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    CASE rtti_get_type_kind( val ).
      ...
      WHEN OTHERS.
        ...
        IF rtti_check_clike( val ) IS NOT INITIAL AND val IS NOT INITIAL.
          ...
          CLEAR temp163.
          temp163-text = val.
          INSERT temp163 INTO TABLE result.
        ENDIF.
    ENDCASE.
  ...
  METHOD msg_get_by_oref.
        DATA temp164 TYPE REF TO cx_root.
        DATA lx LIKE temp164.
        DATA temp165 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    DATA obj TYPE REF TO object.
    DATA lr_tab TYPE REF TO data.
    FIELD-SYMBOLS <tab> TYPE any.
    DATA temp166 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    TRY.
        ...
        CLEAR temp165.
        temp165-type = `E`.
        temp165-text = lx->get_text( ).
        INSERT msg_get_by_oref_attri( io_obj = lx
                                      is_msg = temp165 ) INTO TABLE result.
        RETURN.
      CATCH cx_root ##NO_HANDLER.
    ENDTRY.
    ...
    INSERT msg_get_by_oref_attri( io_obj = obj
                                  is_msg = temp166 ) INTO TABLE result.
  ...
  METHOD msg_get_by_oref_attri.
    ...
    DATA temp167 LIKE LINE OF lt_attri_o.
    DATA ls_attri_o LIKE REF TO temp167.
  ...
  METHOD check_is_rap_struct.
    ...
    DATA lt_attri TYPE abap_component_tab.
    DATA temp168 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp168.
          DATA temp169 TYPE REF TO cl_abap_tabledescr.
          DATA lo_line TYPE REF TO cl_abap_datadescr.
          DATA temp170 TYPE REF TO cl_abap_structdescr.
          DATA lt_comps TYPE abap_component_tab.
          DATA temp171 LIKE LINE OF lt_comps.
          DATA ls_comp LIKE REF TO temp171.
    lt_attri = rtti_get_t_attri_by_any( val ).
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      " the component's descriptor is already in the attribute table - no
      " ASSIGN and no describe_by_data per component
      CHECK ls_attri->type->kind = cl_abap_typedescr=>kind_table.

      TRY.

          temp169 ?= ls_attri->type.

          lo_line = temp169->get_table_line_type( ).
          CHECK lo_line->kind = cl_abap_typedescr=>kind_struct.

          temp170 ?= lo_line.

          lt_comps = temp170->get_components( ).
  ...
  METHOD msg_get_rap.
    ...
    DATA lt_attri TYPE abap_component_tab.
    DATA temp172 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp172.
      FIELD-SYMBOLS <tab> TYPE any.
      FIELD-SYMBOLS <ftab> TYPE ANY TABLE.
      DATA temp173 TYPE REF TO cl_abap_tabledescr.
      DATA lo_line TYPE REF TO cl_abap_datadescr.
      FIELD-SYMBOLS <row> TYPE ANY.
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      ...
      temp173 ?= ls_attri->type.

      lo_line = temp173->get_table_line_type( ).
  ...
  METHOD msg_get_rap_row.
    ...
    FIELD-SYMBOLS <fail> TYPE any.
      FIELD-SYMBOLS <cause> TYPE any.
        ...
        DATA temp174 TYPE z2ui5_cl_ui5_util_context=>ty_s_msg.
    ...
    IF sy-subrc = 0.
      ...
      IF sy-subrc = 0.
        ...
        CLEAR temp174.
        temp174-type = `E`.
        temp174-text = lv_text.
        temp174-t_meta = lt_meta.
        INSERT temp174 INTO TABLE messages.
      ENDIF.
    ENDIF.
  ...
  METHOD scan_flag_prefix.
    ...
    DATA lv_len TYPE i.
    DATA lt_attri TYPE abap_component_tab.
    DATA temp175 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp175.
  ...
  METHOD msg_get_rap_action.
    ...
    READ TABLE lt_suffix INTO temp177 INDEX 1.
    IF sy-subrc = 0.
      temp176 = temp177.
    ENDIF.
    result = temp176.
  ...
  METHOD msg_get_rap_flatten.
    ...
    DATA temp178 LIKE LINE OF lt_attri.
    DATA ls_attri LIKE REF TO temp178.
      FIELD-SYMBOLS <comp> TYPE any.
      DATA lv_sub_kind TYPE string.
        DATA lv_sub TYPE string.
            DATA lv_str TYPE string.
            DATA temp179 LIKE LINE OF lt_part.
    lv_kind = rtti_get_type_kind( val ).
    IF lv_kind <> cl_abap_datadescr=>typekind_struct1
        AND lv_kind <> cl_abap_datadescr=>typekind_struct2.
      RETURN.
    ...
    LOOP AT lt_attri REFERENCE INTO ls_attri.
      ...
      ELSEIF <comp> IS NOT INITIAL AND rtti_check_printable( <comp> ) = abap_true.
        ...
        TRY.
            ...
            temp179 = |{ ls_attri->name }={ lv_str }|.
            APPEND temp179 TO lt_part.
          CATCH cx_root ##NO_HANDLER.
        ENDTRY.
      ENDIF.
    ENDLOOP.
  ...
  METHOD msg_get_rap_meta.
    ...
    DATA lv TYPE string.
      DATA temp180 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp181 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp182 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp183 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp184 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
      DATA temp185 TYPE z2ui5_cl_ui5_util_context=>ty_s_name_value.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp180-n = `element`.
      temp180-v = lv.
      INSERT temp180 INTO TABLE result.
    ENDIF.
    ...
    IF lv IS NOT INITIAL.
      ...
      CLEAR temp181.
      temp181-n = `state_area`.
      temp181-v = lv.
      INSERT temp181 INTO TABLE result.
    ENDIF.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp182-n = `action`.
      temp182-v = lv.
      INSERT temp182 INTO TABLE result.
    ENDIF.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp183-n = `pid`.
      temp183-v = lv.
      INSERT temp183 INTO TABLE result.
    ENDIF.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp184-n = `cid`.
      temp184-v = lv.
      INSERT temp184 INTO TABLE result.
    ENDIF.
    ...
    IF lv IS NOT INITIAL.
      ...
      temp185-n = `tky`.
      temp185-v = lv.
      INSERT temp185 INTO TABLE result.
    ENDIF.
  ...
  METHOD msg_get_rap_fail_text.
    ...
    DATA temp186 TYPE string.
    CASE cause.
      WHEN 0.
        temp186 = `Operation failed`.
      WHEN 1.
        temp186 = `Entity not found`.
      WHEN 2.
        temp186 = `Entity is locked`.
      WHEN 3.
        temp186 = `Authorization failure`.
      WHEN 4 OR 5.
        temp186 = `Concurrent modification`.
      WHEN 6.
        temp186 = `Operation disabled`.
      WHEN 7.
        temp186 = `Operation forbidden`.
      WHEN 8.
        temp186 = `Semantic error`.
      WHEN 9.
        temp186 = `Determination failed`.
      WHEN 10.
        temp186 = `Permission denied`.
      WHEN 11.
        temp186 = `Validation failed`.
      WHEN OTHERS.
        temp186 = |Operation failed (cause code { cause })|.
    ENDCASE.
    result = temp186.
