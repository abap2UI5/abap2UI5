...
*         e_t_where      = lt_where
  ...
  METHOD msg_get_by_msg.
    ...
    DATA temp150 TYPE z2ui5_cl_util=>ty_s_msg.
  ...
  METHOD msg_box_format.
    ...
    DATA lt_msg TYPE z2ui5_cl_util=>ty_t_msg.
      DATA temp161 LIKE LINE OF lt_msg.
      DATA temp162 LIKE sy-tabix.
      DATA temp163 LIKE LINE OF lt_msg.
      DATA temp164 LIKE sy-tabix.
      DATA temp165 LIKE LINE OF lt_msg.
      DATA temp166 LIKE sy-tabix.
      DATA lt_detail_items TYPE string_table.
      DATA temp167 LIKE LINE OF lt_msg.
      DATA lr_msg LIKE REF TO temp167.
        DATA temp168 LIKE LINE OF lt_detail_items.
      DATA temp169 LIKE LINE OF lt_msg.
      DATA temp170 LIKE sy-tabix.
      DATA temp171 LIKE LINE OF lt_msg.
    ...
    IF lines( lt_msg ) = 1.
      ...
      temp162 = sy-tabix.
      READ TABLE lt_msg INDEX 1 INTO temp161.
      sy-tabix = temp162.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-text  = temp161-text.
      
      
      temp164 = sy-tabix.
      READ TABLE lt_msg INDEX 1 INTO temp163.
      sy-tabix = temp164.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-type  = to_lower( ui5_get_msg_type( temp163-type ) ).
      
      
      temp166 = sy-tabix.
      READ TABLE lt_msg INDEX 1 INTO temp165.
      sy-tabix = temp166.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-title = msg_get_title( temp165-type ).
    ...
    ELSEIF lines( lt_msg ) > 1.
      ...
      LOOP AT lt_msg REFERENCE INTO lr_msg.
        ...
        temp168 = |<li>{ lr_msg->text }</li>|.
        INSERT temp168 INTO TABLE lt_detail_items.
      ENDLOOP.
      result-details = `<ul>` && concat_lines_of( lt_detail_items ) && `</ul>`.
      
      
      temp170 = sy-tabix.
      READ TABLE lt_msg INDEX 1 INTO temp169.
      sy-tabix = temp170.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-title   = msg_get_title( temp169-type ).
      
      
      temp172 = sy-tabix.
      READ TABLE lt_msg INDEX 1 INTO temp171.
      sy-tabix = temp172.
      IF sy-subrc <> 0.
        ASSERT 1 = 0.
      ENDIF.
      result-type    = ui5_get_msg_type( temp171-type ).
  ...
  METHOD rtti_check_serializable.
        DATA temp173 TYPE REF TO if_serializable_object.
        DATA lo_dummy LIKE temp173.
  ...
  METHOD app_get_url.
    ...
    DATA temp174 TYPE z2ui5_cl_util=>ty_s_name_value.
    lt_param = url_param_get_tab( search ).
    DELETE lt_param WHERE n = `app_start`.
    
    CLEAR temp174.
    temp174-n = `app_start`.
    temp174-v = to_lower( classname ).
    INSERT temp174 INTO TABLE lt_param.
