...
GET REFERENCE OF <temp7> INTO lr_filter.
  ...
  METHOD tab_get_where_by_dfies.
    ...
    DATA temp17 LIKE LINE OF it_dfies.
    DATA dfies LIKE REF TO temp17.
      FIELD-SYMBOLS <row> TYPE data.
...
DATA lt_cols TYPE temp2.
  ...
  METHOD rtti_get_t_attri_by_any.
    ...
    DATA comps TYPE abap_component_tab.
    DATA temp36 LIKE LINE OF comps.
...
|x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,| &&
  ...
  METHOD url_param_get.
    ...
    READ TABLE lt_params INTO temp43 WITH KEY n = lv_val.
    IF sy-subrc = 0.
      temp42 = temp43-v.
    ENDIF.
    result = temp42.
...
DATA lt_param TYPE temp3.
    DATA temp45 LIKE LINE OF lt_param.
...
*                      v = c_trim_lower( lv_value ) ) INTO TABLE rt_params.
      ...
      temp46-n = lv_name.
      temp46-v = lv_value.
      INSERT temp46 INTO TABLE rt_params.
    ENDLOOP.
  ...
  METHOD url_param_set.
    ...
    IF sy-subrc <> 0.
      ...
      CLEAR temp48.
      temp48-n = lv_n.
      temp48-v = c_trim_lower( value ).
      INSERT temp48 INTO TABLE lt_params.
    ENDIF.
  ...
  METHOD rtti_get_t_attri_by_table_name.
        ...
        DATA temp49 TYPE REF TO cl_abap_structdescr.
        DATA lo_struct LIKE temp49.
    ...
    DATA lr_comp LIKE REF TO temp52.
      DATA lt_attri TYPE abap_component_tab.
    ...
    TRY.
      ...
      CATCH cx_root.
        ...
        TRY.
            ...
            temp51 ?= lo_tab->get_table_line_type( ).
            lo_struct = temp51.
          CATCH cx_root.
            RETURN.
        ENDTRY.
  ...
  METHOD filter_get_sql_where.
    ...
    DATA ls_filter LIKE LINE OF val.
      DATA temp53 LIKE REF TO ls_filter-t_range.
DATA lo_range TYPE REF TO z2ui5_cl_util_range.
    LOOP AT val INTO ls_filter.

      
      GET REFERENCE OF ls_filter-t_range INTO temp53.

CREATE OBJECT lo_range TYPE z2ui5_cl_util_range EXPORTING iv_fieldname = ls_filter-name ir_range = temp53.
  ...
  METHOD rtti_create_tab_by_name.
    ...
    DATA temp55 TYPE REF TO cl_abap_datadescr.
    DATA data_desc LIKE temp55.
  ...
  METHOD msg_get.
    ...
    DATA temp56 LIKE LINE OF lt_msg.
    DATA temp57 LIKE sy-tabix.
    lt_msg = msg_get_t( val ).
    
    
    temp57 = sy-tabix.
    READ TABLE lt_msg INDEX 0 INTO temp56.
    sy-tabix = temp57.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
