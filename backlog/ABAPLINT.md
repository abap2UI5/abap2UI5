...
| Item | What | Priority | In stock since | Upstream |
|---|---|---|---|---|
| [`abaplint-default-key-implicit`](items/abaplint-default-key-implicit.md) | '`DATA t TYPE TABLE OF x.` with no `WITH … KEY` gets the same default key as `WITH DEFAULT KEY`; `avoid_use.defaultKey` reports only the spelled-out form, but `fully_type_itabs` already reports the implicit one ("Specify table type" / "Specify table key") — deferred, there is nothing left to file'<br><sub>written up in [3. Extended check (SLIN/ATC) — runs in real systems, not here](../.claude/skills/abap-check/SKILL.md)</sub><br><sub>measured 2026-09-19: fires on <b>31</b> site(s) in abap2UI5, 12 correct look-alike(s)</sub> | low | 2026-09-19 | abaplint/abaplint |
