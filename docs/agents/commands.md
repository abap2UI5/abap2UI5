...
| `npm run check:members` | Fail when any class — production or test — reads a PRIVATE/PROTECTED member of ANOTHER class it is neither a friend nor (for PROTECTED) a subclass of: `ref->member` with `ref` typed through a method-local declaration, a parameter or an attribute, and `class=>member` (`.github/scripts/member-visibility-gate.mjs`; part of `verify`, gated in `check_gates.yaml`). abaplint and the transpiler do not check attribute visibility, so `mv_session_sticky` reached a user's system as *Field "MV_SESSION_STICKY" is unknown* (2026-09-23). Scans all of `src/` |
...
| `npm run check:atc` | The extended-check (SLIN/ATC) gate — a sequential read without `"#EC CI_SORTSEQ` (`LOOP AT … WHERE`, `READ TABLE … WITH KEY`, and a table expression keyed on a component — the last two in production code only, since test classes hold 76 of the 86 matches and none of them is shipped), an empty `CATCH` without `##NO_HANDLER`, `FIND`/`REPLACE … REGEX` without `##REGEX_POSIX`, a `SELECT` with no `WHERE` clause and no `"#EC CI_NOWHERE`, a text symbol (`'text'(001)`) passed to a parameter, a range-table `VALUE` row without `sign`/`option` or with a literal outside their domain (`'eq'`, `'ZZ'`), and an ABAP Doc block that documents nothing (before a chain keyword, inside a parameter list, before a section end, or separated from its declaration by a plain comment). Scoped to this repository's own ABAP (`src/00/01`, `src/00/02` are upstream mirrors, `src/99` is frozen). abaplint models none of these (part of `verify`, gated in `check_gates.yaml`; background in `.claude/skills/abap-check/SKILL.md`) |
...
| `npm run check:audit` | `npm audit` over both lockfiles (root and `app/`) against the accepted-advisory list in `.github/scripts/npm-audit-gate.mjs` — each accepted GHSA id carries the reason it can stay; fails on a new high/critical advisory or on an accepted one the audit no longer reports, and says so and passes when the registry is unreachable. Not part of `gates`/`verify` (it asks the registry about the whole tree, and a new advisory would turn every unrelated pull request red); `npm-audit.yaml` runs it weekly and on lockfile changes |
...
| `npm run check:types` | TypeScript `checkJs` over `app/webapp/**` (`app/tsconfig.json`, no emit, nothing shipped changes). No UI5 typings on purpose: they describe the current release, and the frontend has to run on 1.71, so `sap` stays `any` (`app/types/globals.d.ts`) and the check covers the frontend's own code and its DOM use. A DOM narrowing is a JSDoc cast, never a code change (part of `verify:full`, gated in `UI5_2X.yaml`, needs `app/node_modules`) |
...
| `npm run check:subrc` | Fail when a `sy-subrc` test's nearest `sy-subrc`-setting statement is a plain dynamic `ASSIGN` (#1937: a SUCCESSFUL assign does not reset `sy-subrc` on every release, so the test reads FALSE for an assignment that worked). `IS [NOT] ASSIGNED` is the fix, with `UNASSIGN <fs>.` first wherever the symbol can already be bound — inside a loop, or assigned earlier in the method. `ASSIGN COMPONENT … OF STRUCTURE` is the negative: there `sy-subrc` IS the documented check. Runs the detector next to `backlog/items/abaplint-subrc-after-assign.md` rather than a copy of it (`.github/scripts/subrc-after-assign-gate.mjs`; part of `verify`, gated in `check_gates.yaml`). Same scope as `check:atc` |
...
| `npm run check:downport` | Fail when a 7.02 built-in function (`to_upper( )`, `substring( )`, …) stands in a `WITH KEY` operand, an internal-table `WHERE`, or the operand of a predicate expression (`condense( val ) IS INITIAL`) — positions that only become general expression positions at 7.40, so the downported statement does not compile on 7.02/7.31 (`.github/scripts/downport-operand-gate.mjs`; part of `verify`, gated in `check_gates.yaml`; #2664). Same scope as `check:atc` |
...
| `npm run downport` | Downport `src/` into `node/downport/` for 7.02 compatibility (non-destructive; the step `verify` runs). Ends by failing on a `DATA … LIKE` a generically typed field symbol in the output — the shape `REF #( <generic fs> )` downports to, which 7.02–7.4x refuse and abaplint's v702 check does not see (`downport-fix.mjs check-generic-like`) |
...
| `npm run pack:backend` | Pack the built backend — `node/downport`, `node/output` and `node/deps` (without their `.git` directories), plus a `backend-manifest.json` at the archive root naming the version, commit, build time, node and transpiler versions — into `backend-<version>.tar.gz` at the repository root (`-- --out <file>` elsewhere). Needs the transpiled tree (`npm run downport && npm run auto_transpile`) and refuses without it, naming the two commands. `backend-prebuilt.yaml` runs it on every published release and attaches the result; the asset name and the manifest are a contract `abap2UI5/mcp-server` reads instead of building the backend itself (`node/setup/pack-backend.mjs`) |
| `npm run pack:node-runtime` | Pack the built framework as the npm package `@abap2ui5/node-runtime` — `node/output`, `node/setup/setup.mjs`, `node/srv/host.mjs` (the entry point), `node/downport` and `node/setup/npm.README.md` under the manifest `node/setup/npm.package.json`, with the version set to the framework's and the two `@abaplint` dependencies pinned to the lockfile's exact versions — into `npm-package/` (`-- --out <dir>` elsewhere). No `webapp/`: the GET page embeds the component. Needs the transpiled tree and refuses without it. `-- --check` then installs the tarball into a scratch project and drives it as a host would (GET / answers with the component embedded, a class the scratch project transpiles against `downport/` registers in the running runtime) — needs the registry, a few minutes. `backend-prebuilt.yaml` runs it with `--check` on every published release and publishes the result (`node/setup/pack-npm.mjs`; AGENTS.md, "The npm packages") |
| `npm run pack:embed-control` | Pack `app/webapp` as the npm package `@abap2ui5/embed-control` — the component `z2ui5` and the control `z2ui5.reuse.Container` unchanged, plus a `Component-preload.js` (and its map, pointed at the unminified sources) built with the UI5 CLI of `app/` — under the manifest, `ui5.yaml` (a UI5 tooling project of type `module`, `/resources/z2ui5/` → `webapp/`) and README in `frontend/npm/`, into `npm-package/` (`-- --out <dir>` elsewhere). Needs `npm --prefix app ci`. `-- --check` installs the tarball into a scratch UI5 app and runs `ui5 build --all` there — the control, the component and its preload have to arrive under `dist/resources/z2ui5/`. `backend-prebuilt.yaml` runs it with `--check` in its `frontend` job on every published release and publishes the result (`tools/pack-frontend.mjs`) |
...
| `npm run express` | Start dev server on port 3000 (`PORT`/`HOST` to change) — through `node/srv/host.mjs`, the entry point of the npm package `@abap2ui5/node-runtime`, so the dev server and the package share one code path |
...
- **The structured suites:** the test includes of the engine's four state carriers (`z2ui5_cl_ui5_srv_model`, `z2ui5_cl_ui5_srv_bind`, `z2ui5_cl_ui5_app_cont`, `z2ui5_cl_ui5_handler`) end in one section per functionality, numbered in the order of a roundtrip (`ltcl_01_dissolve` … `ltcl_05_draft`; `ltcl_01_path` … `ltcl_03_options`; `ltcl_01_xml` … `ltcl_04_model`; `ltcl_01_request` … `ltcl_04_nav`), every section inheriting fixture, helpers and invariants from an abstract `ltcl_00_base`. **A new attribute form is one more attribute in the fixture** (`ltcl_app_shapes`, S01–S30, every form an app attribute can take); every section picks it up. The classes above the section header are the fixtures the sections share (apps, helpers, a filter) - the incident-by-incident test classes that used to sit there were deleted once their `OBSOLETE` notes had been checked against the sections, so a new case goes into a section, never into a class of its own. The `abap_transpile.json` skips name the section methods
...
- **A PRIVATE/PROTECTED member is not reachable from another class** — not from production code, not from the test class of a different class. Make it PUBLIC (`READ-ONLY` where only the owner writes it) rather than reaching in. Gated by `npm run check:members` (`.github/scripts/member-visibility-gate.mjs`).
