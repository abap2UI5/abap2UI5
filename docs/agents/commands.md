...
| `npm run pack:npm` | Pack the built framework as the npm package `@abap2ui5/node` — `node/output`, `node/setup/setup.mjs`, `node/srv/host.mjs` (the entry point), `app/webapp`, `node/downport` and `node/setup/npm.README.md` under the manifest `node/setup/npm.package.json`, with the version set to the framework's and the two `@abaplint` dependencies pinned to the lockfile's exact versions — into `npm-package/` (`-- --out <dir>` elsewhere). Needs the transpiled tree and refuses without it. `-- --check` then installs the tarball into a scratch project and drives it as a host would (GET / answers, `webappDir` finds the component, a class the scratch project transpiles against `downport/` registers in the running runtime) — needs the registry, a few minutes. `backend-prebuilt.yaml` runs it with `--check` on every published release and publishes the result (`node/setup/pack-npm.mjs`; AGENTS.md, "The transpiled framework is a package") |
| `npm run pack:frontend` | Pack `app/webapp` as the npm package `@abap2ui5/frontend` — unchanged, plus a `Component-preload.js` (and its map, pointed at the unminified sources) built with the UI5 CLI of `app/` — under the manifest, `ui5.yaml` (a UI5 tooling project of type `module`, `/resources/z2ui5/` → `webapp/`) and README in `frontend/npm/`, into `npm-package/` (`-- --out <dir>` elsewhere). Needs `npm --prefix app ci`. `-- --check` installs the tarball into a scratch UI5 app and runs `ui5 build --all` there — the component and its preload have to arrive under `dist/resources/z2ui5/`. `backend-prebuilt.yaml` runs it with `--check` in its `frontend` job on every published release and publishes the result (`tools/pack-frontend.mjs`) |
...
| `npm run express` | Start dev server on port 3000 (`PORT`/`HOST` to change) — through `node/srv/host.mjs`, the entry point of the npm package `@abap2ui5/node`, so the dev server and the package share one code path |
