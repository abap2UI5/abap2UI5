...
| `docs/agents/` | The reference material `AGENTS.md` points at rather than carries: `building-apps.md` — the in-repo app-building guide (gated by `npm run check:guide`, and mirrored into `app-template`'s `AGENTS.md`, which `npm run check:shared` holds to it); `repository-map.md` — this file; `ci-workflows.md` — what every workflow does, plus the two deliveries of the transpiled backend (`@abap2ui5/runtime`); `test-inventory.md` — which frontend module has which JS unit spec; `architecture-seams.md` — the swappable draft store and serializer and the codepage fallback; `decisions.md` — the design decisions AGENTS.md keeps only the prohibition of; `api-snapshot-exceptions.md` — the recorded rule-5 exceptions |
...
| the times the public-API snapshot was regenerated over a change | `docs/agents/api-snapshot-exceptions.md` |
| the extended-check (SLIN/ATC) traps | `.claude/skills/abap-check/SKILL.md`, section 3 |
