...
|---|---|---|
| `core/Lib.js` | `buildDeltaFromPaths.spec.js`, `utilHelpers.spec.js`, `sizeLimit.spec.js` | `utilHelpers` incl. `logError`'s ring and its mirror into `sap/base/Log` (component `z2ui5`, never a throw without UI5) |
...
| `cc/LPTitle.js` | `lpTitle.spec.js` | the control delegates to the REAL `SET_TITLE_LAUNCHPAD` handler of `core/actions/Launchpad.js` (loaded alongside), so the standalone no-op, the toText normalization and the failure logging run through the action |
| `cc/Favicon.js` | `favicon.spec.js` | the control delegates to the REAL `SET_FAVICON` handler of `core/actions/Browser.js` (loaded alongside): which `<link>` is written, the URL guard, the suppressed invalidation |
...
| `core/actions/Shortcuts.js` | `shortcuts.spec.js`, `frontendAction.spec.js` (through the dispatch) | the dedicated spec drives the two pure halves directly - `normalizeShortcut` (every spelling, the aliases, modifiers-only, the `+` key as `Ctrl++`) and `shortcutFromEvent` - and pins that both agree for the same combination, plus the scope precedence (a control scope over a slot scope over the unscoped entry, the innermost open slot first); the composed spec covers the registry through the keydown listener. `KEYBOARD_SET_MODE` used to be the module's second handler and was removed on 2026-09-22 (`cc/InputExt.js` carries `inputmode` as a bound property instead, covered by `inputExt.spec.js`) |
| `core/actions/Variants.js` | `frontendAction.spec.js` (through the dispatch) | `SMART_VARIANT_INIT` incl. the retry chain and its once-per-key guard - which lives on the context (`ctx.variants.activeInits`), so two components keep their own chains for the same ids - and `FILTER_BAR_VARIANT_INIT` (its own describe block) |
| `core/actions/ViewOps.js` | `frontendAction.spec.js` (through the dispatch), `focus-after-enable` e2e | `SET_FOCUS` (incl. a focus deferred to a rendering dropped once its controller is gone) and `START_TIMER` here; `SET_ODATA_MODEL`'s destroy handshake and the on-first-use client load are in `view1Events.spec.js` (the OData ownership block) |
| the URL-shaped handlers of `core/actions/Browser.js` through the REAL `core/Lib.js` validators | `browserActions.spec.js` | `DOWNLOAD_B64_FILE`'s protocol guard, active-`data:`-MIME block and filename sanitizer; `OPEN_NEW_TAB`'s same-origin guard and cleared `window.opener`; `URLHELPER`'s CR/LF header-injection block and the `REDIRECT` protocol guard; `LOCATION_RELOAD`'s same-origin guard; `SYSTEM_LOGOUT` (the launchpad logout, the BSP terminate iframe with its load and safety-net finish, the ICF logoff fallback, a refused URL); `PLAY_AUDIO` (protocol guard, a rejected or throwing `play( )` caught into the log) |
...
| the `action( )` entry point of `core/actions/Slots.js` (the VIEW_SLOTS target) | `slotsAction.spec.js` | the argument shapes every slot has to survive - a display without options, the popover anchor, a superseded display; the serialized build chain the fragment slots share with MAIN (`chainBuild`: a second popup waits for the first fragment to load, a build a restore superseded is discarded and the restore's runs after it); the nested display's `methodDestroy` / `methodInsert` failure branches. The model fan-out is covered by `view1Events.spec.js`, the display internals only through it |
| `controller/View1.controller.js` event handling (incl. the `eB` busy guard, the event-array guard, and the one-slot `check_queue_last` queue: last firing kept, dispatched after the response, left to a superseding request, dropped with its controller and on an app switch), the after-render phase (model push by MODEL presence, per-response router sync, a rejected async follow-up action logged and the rest still run, a superseded response's failed build raising no overlay) and the `core/actions/Slots.js` model fan-out, plus the two event-argument helpers that reach the live control tree (`textPath( )`, and `slotById( )` / `slotValue( )` for a control in another view slot) | `view1Events.spec.js` | `slotValue( )`'s whole point is that it cannot throw — an argument expression is evaluated while UI5 dispatches the handler, so a throw there loses the event and not just the value; four of its five cases are miss paths |
| `core/Server.js` timeout handling | `serverTimeout.spec.js` | `createTimeoutSignal` on both engines, and the abort path end to end: the rejected fetch reported as the timeout with a Retry that re-arms busy and re-sends the identical body |
| `core/Server.js` answer side of `readHttp` | `serverReadHttp.spec.js` | an HTTP status outside 2xx (the body as the text, the status for an empty or unreadable body, Retry on 502/503/504 and not on a 500), invalid JSON, a body without `S_FRONT`, the `PROTOCOL` mismatch / match / absence, the `sap-contextid` adoption |
| `core/Server.js` request sequencing, the value-aware clear of a winning request's sent paths (`_clearSentPaths`) and the queued-event drop on `reset( )` / `responseError` / `restoreFromRoute( )` | `serverRequestSeq.spec.js` | — |
